...
class Primorye24Parser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'http://primorye24.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/news/archive/{$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $itemXPath = '//div[contains(@class, "news-feed__item")] | //div[contains(@class, "news-feed")]/preceding-sibling::article';
            ...
            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $uriPreviewPage) {
                $titleCrawler = $newsPreview->filterXPath('//p[contains(@class, "news-feed__title")]/a | //h1/a');
                $uri = UriResolver::resolve($titleCrawler->attr('href'), $uriPreviewPage);

                $previewList[] = new NewsPostDTO($uri, null, $titleCrawler->text(), null);
            });
        }
    ...
    private function parseNewsPage(NewsPostDTO $previewNewsItem): NewsPost
    {
        ...
        $description = $newsPostCrawler->filterXPath('//div[contains(@class,"article-page-heading")]//h2')->text();

        $publishedAtString = trim($newsPostCrawler->filterXPath('//span[contains(@class, "post-meta")]')->text());
