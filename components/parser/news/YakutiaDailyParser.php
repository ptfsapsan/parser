...
/**
 * News parser from site https://yakutia-daily.ru/
 * @author jcshow
 */
class YakutiaDailyParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://yakutia-daily.ru';
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        self::removeNodes($crawler, 
            '//comment() | //br | //hr | //script | //style | //head 
            | //header | //meta | //link | //title | //noscript | //form'
        );
        self::removeNodes($crawler, '//div[contains(@class, "pvc_clear")]');
        self::removeNodes($crawler, '//p[contains(@class, "pvc_stats")]');
        self::removeNodes($crawler, '//div[contains(@class, "daftplugPublic")]');
        self::removeNodes($crawler, '//div[contains(@class, "pageviews-wrapper")]/following-sibling::*');
        self::removeNodes($crawler, '//div[contains(@class, "pageviews-wrapper")]');

        //Get image if exists
        $picture = null;
        $imageBlock = $crawler->filterXPath('
            //div[contains(@class,"elementor-image")]
            //img[1]
        ')->getNode(0);
        ...
        while ($description === '') {
            $descriptionBlock = $crawler->filterXpath('
                //div[contains(@data-widget_type,"theme-post-content.default")]
