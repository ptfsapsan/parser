...
class MyaltaiComParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://myaltai.com';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/topics/lenta/page/{$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler = $previewNewsCrawler->filterXPath('//main[contains(@class,"main")]/section[contains(@class,"mdl-grid")][1]/article');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $title = $this->trimNormalize($newsPreview->filter('.box-content a')->text());
                $uri = UriResolver::resolve($newsPreview->filter('.box-content a')->attr('href'), $this->getSiteUri());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $publishedAt = $this->getPublishedAt($newsPageCrawler);
        ...
        $contentCrawler = $newsPageCrawler->filter('.mdl-card article.article > .content');

        $this->removeDomNodes($contentCrawler, '//script | //video | //style | //form | //table');
        $this->removeDomNodes($contentCrawler, '//*[contains(@class,"adsense-single")]');
        $this->removeDomNodes($contentCrawler, '//*[text()="*реклама"]/parent::p//following-sibling::*');
        $this->removeDomNodes($contentCrawler, '//*[text()="* реклама"]/parent::p//following-sibling::*');
        $this->removeDomNodes($contentCrawler, '//*[text()="*реклама"]/parent::p');
        $this->removeDomNodes($contentCrawler, '//*[text()="* реклама"]/parent::p');
        $this->removeDomNodes($contentCrawler, '//*[contains(@class,"article-read-more-title")]/following-sibling::p');
        $this->removeDomNodes($contentCrawler, '//*[contains(@class,"article-read-more-title")]');
    ...
    private function trimNormalize(string $string): string
    {
        return Text::trim($this->normalizeSpaces($string));
    }
    ...
    private function getPublishedAt(Crawler $crawler): ?DateTimeImmutable
    {
        ...
        $publishedAt = $this->trimNormalize($crawler->filter('article.article time.article-date')->text());
    ...
    private function getDescriptionFromText(Crawler $crawler): string
    {
        ...
        foreach ($tags as $item) {
            if ($crawler->filterXPath("//{$item}")->count()) {
                $tag = $item;
                break;
            }
        }
        ...
        for ($deep = 1; $deep <= 10; $deep++) {
            if ($description && (str_contains($description, '.') || mb_strlen($description) > 100)) {
                break;
            }

            $descriptionCrawler = $crawler->filterXPath("//{$tag}[$deepForFilter]");
            if ($this->crawlerHasNodes($descriptionCrawler)) {
                $descriptionText = $descriptionCrawler->text();
                if ($descriptionText) {
                    $description .= " $descriptionText";
                    $this->removeDomNodes($crawler, "//{$tag}[$deepForFilter]");
                } else {
                    $deepForFilter++;
                }
