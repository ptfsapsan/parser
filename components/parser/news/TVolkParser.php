...
class TVolkParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://tvolk.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("div.news-page div.news-page__content > a img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl(self::ROOT_SRC . $picHolder->attr("src"));
        }

        $picHolder = $crawler->filter("div.news-page div.news-page__content div#scroller-photo ul li img");
        if ($picHolder->count() !== 0) {
            $picHolder->each(function (Crawler $picNode) use ($post) {
                if ($post->image === null) {
                    $post->image = self::normalizeUrl(self::ROOT_SRC . $picNode->attr("src"));
                } else {
                    self::addImage($post, self::normalizeUrl(self::ROOT_SRC . $picNode->attr("src")));
                }
            });
        ...
        $body = $crawler->filter("div.news-page div.news-page__content div.news__preview_text");
