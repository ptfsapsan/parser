...
class BanksTodayParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://bankstoday.net";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $descriptionHolder = $crawler->filter("article div.articleBody div.introContent");
        if ($descriptionHolder->count() === 0) {
            throw new Exception("Не найден блок описания: " . $url);
        }
        $post->description = Helper::prepareString($descriptionHolder->text());

        $picHolder = $crawler->filter("article div.articleBody div#postImgAuthor img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl($picHolder->attr("data-wpfc-original-src"));
            if (!empty($picHolder->attr("data-author"))) {
                self::addText($post, $picHolder->attr("data-author"));
            }
        }


        $body = $crawler->filter("article div.articleBody div.postBody");
        if ($body->count() === 0) {
            $body = $crawler->filter("article div.articleBody");
        }
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if (
                $node->matches("div.code-block")
                || $node->matches("div#postImgAuthor")
                || $node->matches("div.introContent")
            ) {
                continue;
            }
            if ($node->matches("p.important") && !empty(trim($node->text(), "\xC2\xA0"))) {
