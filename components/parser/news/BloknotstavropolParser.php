...
class BloknotstavropolParser extends MediasferaNewsParser implements ParserInterface
{
    ...
    public const ARTICLE_BREAKPOINTS = [
        ...
        'href' => [
            'https://www.instagram.com/bloknot_stavropol/' => false,
            'https://www.facebook.com/bloknot26/' => false,
            'https://t.me/bloknot_stavropol' => false,
            'https://vk.com/bloknot26' => false,
            'https://ok.ru/bloknot26' => false,
        ],
        'id' => [
            'pollFrame' => true,
        ]
    ...
    public static function run(): array
    {
        ...
        $listCrawler->filterXPath(self::NEWSLIST_POST)->slice(0, self::NEWS_LIMIT)->each(function (Crawler $node) use (&$posts) {
            ...
            foreach ($newsPost->items as $key => $item) {
                ...
                else if(strpos($text, 'Попали в сложную ситуацию') !== false) {
                    unset($newsPost->items[$key]);
                    $removeNext = true;
                }
                else if(strpos($text, 'А также подписывайтесь') !== false) {
                    unset($newsPost->items[$key]);
                    $removeNext = true;
                }
            }
