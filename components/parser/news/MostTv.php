 ...
 *
 */
class MostTv extends Aleks007smolBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const MAIN_PAGE_URI = 'https://most.tv/rss';

    /**
     * CSS класс, где хранится содержимое новости
     */
    const BODY_CONTAINER_CSS_SELECTOR = '#intro';
     ...
     */
    const FEED_URL = 'https://most.tv/rss/';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function ($node) use (&$curl, &$posts) {
             ...
             */
            $newsContent = $curl->post($newPost->original);
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $mainImage = (new Crawler($newsContent))->filter('.jeg_featured a');
                ...
                $newsContent = (new Crawler($newsContent))->filter('#intro')->nextAll();
