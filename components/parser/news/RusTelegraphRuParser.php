...
class RusTelegraphRuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://rustelegraph.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve('/rss/full.xml', $this->getSiteUri());
        ...
        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
            ...
            $preview = $newsPreview->filterXPath('//content:encoded')->text();
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $imageNode = $newsPageCrawler->filterXPath('//div[@class="article-one"]//div[contains(concat(" ",normalize-space(@class)," ")," icon ")]//img');
        $image = null;
        if ($this->crawlerHasNodes($imageNode)) {
            $image = UriResolver::resolve($imageNode->attr('src'), $this->getSiteUri());
        }
        ...
        | //p[child::a[contains(@href, "t.me")]][child::span]');
