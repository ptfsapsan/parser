...
/**
 * News parser from site https://cheltoday.ru/
 * @author jcshow
 */
class ChelyabinskTodayParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://cheltoday.ru';
    ...
    public static function getNewsData(): array
    {
        ...
        /** Get RSS news list */
        $curl = Helper::getCurl();
        $newsList = $curl->get(static::SITE_URL . "/rss/");
        if (! $newsList) {
    ...
    public static function getPostDetail(SimpleXMLElement $item): NewsPost
    {
        ...
        self::removeNodes($crawler, '//comment() | //br | //hr | //script | //style | //form 
        | //header | //head | //noscript');
        self::removeNodes($crawler, 'div[contains(@class,"caption")]');
        self::removeNodes($crawler, 'a[contains(@href,"javascript:void(0)")]');
        self::removeNodes($crawler, 'div[@id="header"]');
        self::removeNodes($crawler, 'div[contains(@class,"metapost")]');
        self::removeNodes($crawler, 'div[contains(@class,"description")]');
        self::removeNodes($crawler, 'div[contains(@class,"context")]');

        $detailPage = $crawler->filter('.post .text')->getNode(0);
