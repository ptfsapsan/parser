...
class PavEdin23RuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://pav-edin23.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/page/{$pageNumber}/", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[@id="content"]//div[starts-with(@id,"post-")]');
            ...
            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $maxNewsCount) {
                ...
                $linkCrawler = $newsPreview->filterXPath('//*[contains(@class,"entry-title")]/a');

                $title = Text::trim($this->normalizeSpaces($linkCrawler->text()));

                $uri = UriResolver::resolve($linkCrawler->attr('href'), $this->getSiteUri());

                $publishedAtString = $newsPreview->filterXPath('//div[contains(@class,"entry-meta")]/span[contains(@class,"meta-date")]')->text();
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPage = $this->normalizeSpaces($newsPage);

        $newsPageCrawler = new Crawler($newsPage);

        $contentCrawler = $newsPageCrawler->filterXPath('//div[contains(@id,"content")]/div[starts-with(@id,"post-")]/div[contains(@class,"entry-content")]');

        $mainImageCrawler = $contentCrawler->filterXPath('//img[1]');

        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($contentCrawler, '//img[1]/parent::a | //img[1]');
        }

        $descriptionCrawler = $contentCrawler->filterXPath('//p/strong[1]');
        if ($this->crawlerHasNodes($descriptionCrawler)) {
            $description = str_replace('&shy;', '', Text::trim($this->normalizeSpaces($descriptionCrawler->text())));
            ...
            if ($this->crawlerHasNodes($descriptionCrawler)) {
                ...
                throw new RuntimeException("Не удалось получить description (url: $uri).");
            }
        }
        ...
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"tptn_counter")]');
