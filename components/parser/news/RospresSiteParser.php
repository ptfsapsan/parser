...
class RospresSiteParser extends AbstractBaseParser
{
    ...
    protected function getPreviewNewsDTOList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve('/?format=feed&type=rss', $this->getSiteUrl());
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $mainImageCrawler = $newsPageCrawler->filterXPath('//h1[contains(@class,"itemTitle")]/following-sibling::img[1]')->first();
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = Text::trim($this->normalizeSpaces($mainImageCrawler->attr('src')));
            $this->removeDomNodes($contentCrawler, '//h1[contains(@class,"itemTitle")]/following-sibling::img[1]');
        } else {
