...
class Gazeta19Parser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://gazeta19.ru";
    ...
    public static function run(): array
    {
        ...
        $items = $crawler->filter("div.allmode-wrapper");
        if ($items->count() === 0) {
    ...
    private static function inflatePost(Crawler $postData): NewsPost
    {
        ...
        $title = $postData->filter("div.allmode-topitem h3.allmode-title a")->text();

        $original = self::normalizeUrl(self::ROOT_SRC . $postData->filter("div.allmode-topitem h3.allmode-title a")->attr("href"));

        $dateHolder = $postData->filter("div.allmode-topitem div.allmode-info span.allmode-date");
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("div#k2Container div.itemBody div.itemImageBlock img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl(self::ROOT_SRC . $picHolder->attr("src"));
        }


        $body = $crawler->filter("div#k2Container div.itemBody div.itemFullText");
