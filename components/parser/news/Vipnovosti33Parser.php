...
/**
 * News parser from site http://www.vipnovosti33.ru/
 * @author jcshow
 */
class Vipnovosti33Parser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'http://www.vipnovosti33.ru';
    ...
    public static function getNewsData(int $limit = 100): array
    {
        ...
        while (self::$parsedCount < $limit) {
            /** Get RSS news list */
            $curl = Helper::getCurl();
            $newsList = $curl->get(static::SITE_URL . "/news/page/$pageNum");
            if (! $newsList) {
            ...
            $news = $newsListCrawler->filterXPath('//div[contains(@class, "topnews")]//div[contains(@class, "single")]');
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        $linkBlock = $itemCrawler->filterXPath('//div[contains(@class, "title")]//a')->getNode(0);
        $link = UriResolver::resolve(self::cleanUrl($linkBlock->getAttribute('href')), static::SITE_URL);

        /** Get title */
        $title = self::cleanText($linkBlock->textContent);

        /** Get item datetime */
        $stringToDate = $itemCrawler->filterXPath('//div[contains(@class, "extra")]')->text();
        $stringToDate = preg_replace('/г\./', '', self::convertDateToEng($stringToDate));
        $createdAt = DateTime::createFromFormat('d F Y H:i', $stringToDate, new DateTimeZone('Europe/Moscow'));
        $createdAt->setTimezone(new DateTimeZone('UTC'));
        $createdAt = $createdAt->format('c');

        /** Get description */
        $description = self::cleanText($itemCrawler->filterXPath('//div[contains(@class, "text")]')->text());

        //Get image if exists
        $picture = null;
        $imageBlock = $itemCrawler->filterXPath('//div[contains(@class, "image")]//img')->getNode(0);
        ...
        $detailPage = $crawler->filterXpath('//div[contains(@class, "topnews")]//div[contains(@class, "newstext")]')->getNode(0);
