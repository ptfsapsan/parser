...
class Fin4UParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://www.fin4u.ru";
    ...
    private static function inflatePost(Crawler $postData): NewsPost
    {
        ...
        if (mb_stripos($postData->filter("a")->attr("href"), "mastertarget") !== false) {
            throw new Exception("Рекламный блок");
        }
        $url = $postData->filter("a")->attr("href");
        if ($url[0] !== "/") {
            $url = "/" . $url;
        }
        $original = self::normalizeUrl(self::ROOT_SRC . $url);
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $dateHolder = $crawler->filter("div#content div.text small");
        if ($dateHolder->count() === 0) {
            throw new Exception("Не найден блок даты публикации: " . $url);
        }
        $dateString = explode("|", $dateHolder->text());
        ...
        if ($body->count() !== 1) {
            $body = $crawler->filter("div#content div.text");
        }
        if ($body->count() === 0) {
        ...
        foreach ($body->getNode(0)->childNodes as $bodyNode) {
            ...
            if (mb_stripos($bodyNode->textContent, "Присоединяйтесь к нам в соцсетях, новости и видео") !== false) {
                break;
            }
            if ($node->matches("h4#comments")) {
                break;
            }

            if ($bodyNode->nodeName === "#text" && !empty(trim($bodyNode->textContent))) {
                if (empty($post->description)) {
                    $post->description = Helper::prepareString(trim($bodyNode->textContent));
                } else {
                    self::addText($post, trim($bodyNode->textContent));
                }
                continue;
            }
