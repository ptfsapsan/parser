...
class CityNewsRuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://city-news.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $links = $previewNewsCrawler->filterXPath('//a[contains(concat(" ",normalize-space(@class)," ")," new ")]');

        $links->each(function (Crawler $newsPreview, $i) use (&$previewList, $previewNewsCrawler) {
            
            $uri = UriResolver::resolve($newsPreview->attr('href'), self::SITE_URL);
            $title = trim($previewNewsCrawler->filterXPath('//span[@class="new__link"]')->eq($i)->text());
            
            $publishedAtString = trim($previewNewsCrawler->filterXPath('//*[@class="new__meta"]')->eq($i)->text());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//*[contains(concat(" ",normalize-space(@class)," ")," detail__preview ")]
        | //*[contains(concat(" ",normalize-space(@class)," ")," detail__text ")]');
