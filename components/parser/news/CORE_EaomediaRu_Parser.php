...
class CORE_EaomediaRu_Parser extends ParserCore implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    ...
    public function __construct()
    {
        $this->config = [
            ...
            'rss'     => [
                ...
                // (обязательный)
                'url'                 => '/export/new/news_main11.rss',
            ...
            'element' => [
                ...
                // (обязательный)
                'container'           => '.col-md-8.nolimit_sticky',

                // css-селектор для основного текста
                // (для заполнения модели NewsPostItem)
                // (обязательный)
                'element-text'        => '.page-content.io-article-body',
                ...
                // !должен содержать конечный аттрибут src! (напsример: img.main-image[src])
                // (заполняется только, если отсутствует в витрине)
                'element-image'       => '',
                ...
                // (опционально)
                'element-quote'       => '.fn-quote',

                // игнорируемые css-селекторы
                // (можно через запятую)
                // (опционально)
                //                'ignore-selectors'    => '.inside_banner.noprint, .page-content__related, #soc_invites_block, #mv-content-roll-3609',
                'ignore-selectors'    => '.inside_banner.noprint, #soc_invites_block, #mv-content-roll-3609',
            ]
        ];
