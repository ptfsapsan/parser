...
/**
 * Парсер новостей из RSS ленты progorodchelny.ru
 *
 */
class ProGorodChelny extends TyRunBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const MAIN_PAGE_URI = 'https://progorodchelny.ru';
     ...
     */
    const FEED_URL = 'https://progorodchelny.ru/rss.xml';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('rss channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function (Crawler $node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $articleContent = $newsContent->filter('.ArticleContent')->children();
                $stopParsing = false;
        ...
        switch ($node->nodeName()) {
            ...
            case 'em':
                if ($newPost->description == '-') {
                    $newPost->description = self::prepareDescription($node->text());
                    break;
                }
