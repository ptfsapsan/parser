...
/**
 * Парсер новостей с сайта https://www.vluki.ru/
 */
class VlukiRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://www.vluki.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="post"]');
            $title = $content->filterXPath('//h1')->text();
            $date = $this->getDate($content->filterXPath('//span[@class="show-for-medium"]')->text());
            $image = null;
            $imgSrc = $content->filterXPath('//div[@class="fancybox"]')->filterXPath('//a[@class="preview"]');
            if (!$imgSrc->getNode(0)) {
                $imgSrc = $content->filterXPath('//div[@class="fotorama"]/a');
            }
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->getNode(0)->getAttribute('href'));
            }

            $description = '';
            $descriptionSrc = $content->filterXPath('//div[@itemprop="articleBody"]');
            ...
            $newContentCrawler = $content->filterXPath('//div[@itemprop="articleBody"]');
    ...
    {
        $date = $this->clearText($date, ['Опубликовано:', 'года', 'г.', 'в']);
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@class="media-object-section"]')->filterXPath('//h3/a');
