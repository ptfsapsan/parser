...
class UglegorskNewsParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    private const LINK = 'http://uglegorsk.news/feed/';
    ...
    {
        $p = $parser->find('.td-post-content');
        $p->find('.mistape_caption')->remove();
        $p = $p->find('p');
        $text = $p->text();
        if (!empty($text)) {
            $post->addItem(
        ...
        $images = $parser->find('.td-post-content .gallery-icon.landscape:gt(0) img');
        if (count($images)) {
