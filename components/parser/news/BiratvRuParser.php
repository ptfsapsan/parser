...
class BiratvRuParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('article .entry-content');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"uglshare")]');

        $image = null;

        $mainImageCrawler = $newsPageCrawler->filterXPath('//article//div[contains(@class,"article-thumbnail")]//img[1]');
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($newsPageCrawler, '//article//div[contains(@class,"article-thumbnail")]');
        }
