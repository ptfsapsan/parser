...
class VestiKalmykiaParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://vesti-kalmykia.ru/';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve("/feed/rss/news?format=feed", $this->getSiteUri());
        ...
        $previewNewsCrawler->each(
            function (Crawler $newsPreview) use (&$previewList) {
                ...
                $uriPath = explode('/',$uriPath['path'],-1);
                if ($uriPath[1] !== 'news') {
                   return false;
                }
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@id="main"]');
        ...
        $this->removeDomNodes(
            $contentCrawler,
            '//*[contains(@class,"item-banner")]/following-sibling::* | //*[contains(@class,"item-banner")]'
        );
