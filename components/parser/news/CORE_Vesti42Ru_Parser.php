...
class CORE_Vesti42Ru_Parser extends ParserCore implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    ...
    public function __construct()
    {
        $this->config = [
            ...
            'rss'     => [
                ...
                // (заполняется только, если отсутствует в карточке)
                'element-image'       => 'enclosure[type="image/jpeg"][url]',
            ...
            'element' => [
                ...
                // (обязательный)
                'container'           => '.one-right-column-layout div',

                // css-селектор для основного текста
                // (для заполнения модели NewsPostItem)
                // (обязательный)
                'element-text'        => '.internal-page-content',
                ...
                // (можно через запятую)
                // (опционально)
                'ignore-selectors'    => 'div[style="color: #999"], h1, div[style="text-align: center; margin: 20pt 0"]  img',
            ]
        ];
