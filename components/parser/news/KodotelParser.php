...
/**
 * News parser from site http://kodotel.ru/
 * @author jcshow
 */
class KodotelParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'http://kodotel.ru';
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        /** Get description */
        $descriptionBlock = $crawler->filterXpath('//p/text()[not(contains(., "Фото"))]')->getNode(0);
        ...
        $detailData = $crawler->filterXpath('//p');

        self::removeNodes($crawler, '//comment() | //br | //hr | //script | //style');
        self::removeNodes($crawler, '//p/text()[contains(., "www.adv.rbc.ru")]');

        // parse detail page for texts
        foreach ($detailData as $node) {
