...
class KineshemecParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://kineshemec.ru";

    const FEED_SRC = "/rss/news/all.xml";
    const LIMIT = 100;
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div[itemprop=\"articleBody\"]");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("div.mb10") && !empty(trim($node->text(), "\xC2\xA0"))) {
            ...
            if ($node->matches("div.txt-txt")) {
                foreach ($node->children() as $innerNode) {

                    $txtnode = new Crawler($innerNode);
                    if ($txtnode->matches("p") && !empty(trim($txtnode->text(), "\xC2\xA0"))) {
                        if (empty($post->description)) {
                            $post->description = Helper::prepareString($txtnode->text());
                        } else {
                            self::addText($post, $txtnode->text());
                        }
                        continue;
                    }

                    if ($txtnode->matches("blockquote") && !empty(trim($txtnode->text(), "\xC2\xA0"))) {
                        self::addQuote($post, $txtnode->text());
                        continue;
                    }
                }
            }
            ...
            if ($node->matches("div.imageGallery") && $node->filter("img")->count() !== 0) {
                $node->filter("img")->each(function (Crawler $imgNode) use ($post) {
                    $src = self::normalizeUrl(self::ROOT_SRC . $imgNode->attr("src"));
                    if ($post->image === null) {
                        $post->image = $src;
                    } else {
                        self::addImage($post, $src);
