...
class GorodscoiTelegrafParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://gorodskoitelegraf.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div.fullstory div.fullstory_content");
        ...
        foreach ($body->getNode(0)->childNodes as $bodyNode) {
            ...
            if ($node->matches("p, div, b, span") && !empty(trim($node->text(), "\xC2\xA0"))) {
                $cleanText = trim(str_ireplace($post->description, "", $node->text()), "\xC2\xA0");
