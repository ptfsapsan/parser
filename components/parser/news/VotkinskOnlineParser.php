...
class VotkinskOnlineParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    ...
    public static function run(): array
    {
        ...
        if (count($items)) {
            ...
            foreach ($items as $item) {
                ...
                $titleParser = PhpQuery::newDocument($item->find('title')->text());
                $a = $titleParser->find('a');
                $title = $a->text();
    ...
    private static function getParser(string $link, Curl $curl): PhpQueryObject
    {
        ...
        return PhpQuery::newDocument("\xEF\xBB\xBF" .  $content);
    }

    private static function setOriginalData(PhpQueryObject $parser, NewsPost $post): NewsPost
    {
        $paragraphs = $parser->find('p.MsoNormal:gt(0)');
        if (count($paragraphs)) {
