...
class BO32RuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'http://bo32.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = $this->getSiteUri();
        ...
        $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[@id="app"]/div[@class="articles-article"]');

        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
            $anchorLink = $newsPreview->filterXPath('//div[@class="title"]//a');
            $title = $anchorLink->text();
            $uri = UriResolver::resolve($anchorLink->attr('href'), $this->getSiteUri());

            $publishedAtString = $newsPreview->filterXPath('//*[@class="date"]')->text();

            $publishedAt = DateTimeImmutable::createFromFormat('d.m.Y', $publishedAtString, new DateTimeZone('Europe/Moscow'));
            $publishedAtUTC = $publishedAt->setTimezone(new DateTimeZone('UTC'));

            $preview = $newsPreview->filterXPath('//div[@class="announce"]//descendant::span[last()]')->text();
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," articles-article ")]');

        $imageNode = $newsPostCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," div_article_photo ")]//img');
        ...
        | //*[contains(concat(" ",normalize-space(@class)," ")," name_article ")]
        | //*[contains(concat(" ",normalize-space(@class)," ")," head ")]');
    ...
    private function searchImageNewsItem(DOMNode $node, PreviewNewsDTO $previewNewsItem): ?NewsPostItem
    {
        ...
        $imageLink = $node->getAttribute('data-src') ?? $node->getAttribute('src');
        ...
        if ($isPicture) {
            ...
            if ($imgCrawler->count()) {
                $imageLink = $imgCrawler->first()->attr('data-src') ?? $imgCrawler->first()->attr('src');
            }
        }
