...
class TVMigRuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://www.tv-mig.ru';
    ...
    private function searchHeadingNewsItem(DOMNode $node): ?NewsPostItem
    {
        ...
        if (preg_match('/Фото/m', $node->textContent)) {
            return null;
        }
    ...
    private function searchTextNewsItem(DOMNode $node): ?NewsPostItem
    {
        ...
        if (preg_match('/(Количество показов)|(^Фото)/m', $node->textContent)) {
            return null;
        }
