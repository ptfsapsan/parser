...
class SayHiMeParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," post-content ")]');
        ...
        $contentCrawler = $newsPostCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," content-page ")]');

        $this->removeDomNodes($contentCrawler, '//*[contains(translate(substring(text(), 0, 14), "ФОТО", "фото"), "фото")]
        | //div[descendant::text()[contains(., "Читайте также")]]
        | //*[contains(text(), "Читайте также")]
        | //blockquote[@class="wp-embedded-content"]/following-sibling::*
