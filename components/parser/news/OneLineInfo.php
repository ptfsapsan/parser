 ...
 *
 */
class OneLineInfo extends Aleks007smolBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const MAIN_PAGE_URI = 'https://1line.info';

    /**
     * CSS класс, где хранится содержимое новости
     */
    const BODY_CONTAINER_CSS_SELECTOR = '.news-detail-content';
     ...
     * в формате RegExp
     */
    const EXCLUDE_CSS_CLASSES_PATTERN = 'alert-secondary';
     ...
     */
    const FEED_URL = 'https://1line.info/rss/';
    ...
    protected static function parseNode(Crawler $node, NewsPost $newPost, int $maxDepth, bool &$stopParsing, $descriptionSentences = []): void
    {
         ...
         */
        if (strpos($node->attr('class'),self::EXCLUDE_CSS_CLASSES_PATTERN) !== false) {
            return;
        }
    ...
    private static function parseParagraph(Crawler $node, NewsPost $newPost, array $descriptionSentences): void
    {
        ...
        if (!empty($node->text()) && count($intersect) < count($nodeSentences)) {
            ...
            /**
             * Удаляем первое предложение т.к. совпадает с description
             */
            $text = explode('. ', $text);
            if (strpos($newPost->description, $text[0]) !== false) {
                unset($text[0]);
            }
