 ...
 *
 */
class Ogrik extends Aleks007smolBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const MAIN_PAGE_URI = 'http://ogirk.ru';


    /**
     * CSS класс, где хранится содержимое новости
     */
    const BODY_CONTAINER_CSS_SELECTOR = '.c-body__content';
     ...
     */
    const FEED_URL = 'http://www.ogirk.ru/rss';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function ($node) use (&$curl, &$posts) {
            ...
            $newPost = new NewsPost(
                ...
                'description',
                DateTime::createFromFormat('U', 0)->format('d-m-Y H:i:s'),
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $mainImage = (new Crawler($newsContent))->filter('.c-media img');

                if ($mainImage->count()) {
                    if ($mainImage->attr('src')) {
                        $newPost->image = (strstr($mainImage->attr('src'), self::MAIN_PAGE_URI));
                    }
                }
