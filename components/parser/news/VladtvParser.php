...
class VladtvParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://vladtv.ru';
    ...
    private function getPageContent(string $uri): string
    {
        ...
        if (str_contains($result, 'bpc=')) {
            preg_match('/bpc=[^;.]+/iu', $result, $matches);

            $cookie = array_shift($matches);
            if (!$cookie) {
                throw new RuntimeException('Не удалось получить Cookie');
            }

            $this->curl->setOption(CURLOPT_HTTPHEADER, ["Cookie: {$cookie}"]);
