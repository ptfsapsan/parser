...
class GorlovkaPravdaComParser implements ParserInterface
{
    use \app\components\helper\metallizzer\Cacheable;

    const USER_ID  = 2;
    const FEED_ID  = 2;
    const SITE_URL = 'https://gorlovka-pravda.com';

    protected static $posts = [];

    public static function run(): array
    {
        $url  = self::SITE_URL.'/rss/content/17/feed.rss';
    ...
    protected static function loadPost($item)
    {
        ...
        self::$posts[] = (new Parser())->fill(
            $post,
            $page->filterXpath('//div[@class="con_text"]/text()[contains(., "Добавляй издание Горловская Правда в свои источники")]/preceding-sibling::node()')
        );
    }
