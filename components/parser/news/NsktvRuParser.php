...
class NsktvRuParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('.news-page > .news-detail');

        $mainImageCrawler = $newsPageCrawler->filter('meta[name="og:image"]');
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            ...
            $this->removeDomNodes($contentCrawler, '//div[contains(@class,"media-block")]/img[1]');
        }
        ...
        $contentCrawler = $contentCrawler->filterXPath('//div[contains(@class,"media-block")] | //div[contains(@class,"detail-text")]');

        $descriptionCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"preview-text")]');
        if ($this->crawlerHasNodes($descriptionCrawler)) {
            $descriptionText = Text::trim($this->normalizeSpaces($descriptionCrawler->text()));
            if ($descriptionText) {
                $description = $descriptionText;
                $this->removeDomNodes($contentCrawler, '//div[contains(@class,"preview-text")]');
            }
        }
