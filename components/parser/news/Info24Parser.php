...
class Info24Parser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://info24.ru";

    const FEED_SRC = "/feed/innernews";
    const LIMIT = 100;
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $body = $crawler->filter("article div.tdb_single_featured_image img");

        if($picHolder->count() !== 0){
            $post->image = self::normalizeUrl($picHolder->attr("src"));
        }

        $dateHolder = $crawler->filter("article time.td-module-date");
        if($dateHolder->count() === 0){
            throw new Exception("Не найден блок с датой публикации: " . $url);
        }
        $createDate = new DateTime($dateHolder->attr("datetime"));
        $createDate->setTimezone(new DateTimeZone("UTC"));
        $post->createDate = $createDate;

        $body = $crawler->filter("article div.td-post-content > div");
