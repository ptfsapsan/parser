...
class StolitsaSuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://stolica-s.su";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $headerHolder = $crawler->filter("article header p");

        if($headerHolder->count() !== 0 && !empty(trim($headerHolder->text(), "\xC2\xA0"))){
            $post->description = Helper::prepareString($headerHolder->text());
        }
        $body = $crawler->filter("article div.content-body");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->filter("img")->count() !== 0) {
                ...
                if($post->image === null){
                    ...
                    if($caption->count() !== 0 && !empty(trim($caption->text(), "\xC2\xA0"))){
