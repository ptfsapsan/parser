 ...
 * @rss_html
 */
class VsednrRuParser extends MediasferaNewsParser implements ParserInterface
{
    ...
    public const SITE_URL = 'https://vsednr.ru/';
    public const NEWSLIST_URL = 'https://vsednr.ru/feed/';
    ...
    public const ARTICLE_IMAGE = '.entry-thumb img';
    public const ARTICLE_TEXT = 'article .entry-content .content';
    ...
    protected static function parseNode(Crawler $node, ?string $filter = null): void
    {
        ...
        if($nodeName == 'div' || $nodeName == 'p') {
            $text = trim($node->text());
            if(strpos($text, 'The post') === 0 && substr_count($text, 'first appeared on')) {
                return;
            }
        }
