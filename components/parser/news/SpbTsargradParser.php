...
class SpbTsargradParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://spb.tsargrad.tv';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/ajax/morenews?page={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $previewNewsCrawler) {
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPost = new NewsPost(self::class, $title, $description, $publishedAt->format('Y-m-d H:i:s'), $uri, null);

        $imagesXpath = '//ul[contains(@class,"article__gallery-main-list")]//img';
        $newsPageCrawler->filterXPath($imagesXpath)->each(function (Crawler $crawler) use ($newsPost) {
            $image = $crawler->attr('src');
            if (!$image || $image === '') {
                return;
            }

            $image = UriResolver::resolve($image, $newsPost->original);
            $image = $this->encodeUri($image);

            if ($newsPost->image === null) {
                $newsPost->image = $image;
                return;
            }
            ...
            $newsPost->addItem(new NewsPostItem(NewsPostItem::TYPE_IMAGE, $alt, $image));
        });

        $contentCrawler = $newsPostCrawler->filterXPath('//div[contains(@class,"only__text")]');
    ...
    private function searchQuoteNewsItem(DOMNode $node): ?NewsPostItem
    {
        ...
        $newsPostItem = new NewsPostItem(NewsPostItem::TYPE_QUOTE, preg_replace('/\s+/iu', ' ', $node->textContent));
    ...
    private function searchHeadingNewsItem(DOMNode $node): ?NewsPostItem
    {
        ...
        $newsPostItem = new NewsPostItem(NewsPostItem::TYPE_HEADER, preg_replace('/\s+/iu', ' ', $node->textContent), null, null, $headingLevel);
        $this->nodeStorage->attach($node, $newsPostItem);
        $this->removeParentsFromStorage($node->parentNode);
    ...
    private function searchTextNewsItem(DOMNode $node): ?NewsPostItem
    {
        ...
        $newsPostItem = new NewsPostItem(NewsPostItem::TYPE_TEXT, preg_replace('/\s+/iu', ' ', $node->textContent));
