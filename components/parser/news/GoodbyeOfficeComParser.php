...
class GoodbyeOfficeComParser implements ParserInterface
{
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            ...
            $previewNewsCrawler = $previewNewsCrawler->filter('#primary .posts > article.post');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $title = $newsPreview->filter('.post-title > a')->text();
                $uri = UriResolver::resolve($newsPreview->filter('.post-title > a')->attr('href'), $this->getSiteUri());

                $publishedAt = $newsPreview->filter('.post-meta time')->attr('datetime');
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        if (!$image) {
            $mainImageCrawler = $contentCrawler->filter('.featured-img-container');

            if ($this->crawlerHasNodes($mainImageCrawler)) {
                $image = $mainImageCrawler->attr('data-loftocean-image');
            }
        }
        ...
        $contentCrawler = $contentCrawler->filter('.post-entry');
