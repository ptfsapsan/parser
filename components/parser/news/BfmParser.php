...
class BfmParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    ...
    public static function run(): array
    {
        ...
        $items = $parser->find('.news-block ul li');
        if (count($items)) {
            $n = 0;
            foreach ($items as $item) {
                ...
                $a = $item->find('a');
                $a->find('i.time-news')->remove();
                $title = trim($a->text());
                $original = $a->attr('href');
                $original = sprintf('%s%s', self::DOMAIN, $original);
                $originalParser = self::getParser($original, $curl);
                $span = $originalParser->find('span.date');
                $span->find('em.tag_filter')->remove();
                $createDate = trim($span->text());
                $createDate = date('d.m.Y H:i:s', self::getTimestampFromString($createDate));
                $image = $originalParser->find('.current-article figure.article-figure img')->attr('src');
                if (empty($image) || !filter_var($image, FILTER_VALIDATE_URL)) {
                    $image = null;
                }
                $description = $originalParser->find('.current-article p.about-article')->text();
                try {
    ...
    {
        $p = $parser->find('.current-article p:not(.about-article)');
        $p->find('script')->remove();
        $p->find('span.see_also')->remove();
