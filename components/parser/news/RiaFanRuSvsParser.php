...
/**
 * Парсер новостей с сайта https://riafan.ru/
 */
class RiaFanRuSvsParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://riafan.ru';
    ...
    public function parse(): array
    {
        ...
        $urlNews = self::SITE_URL . '/region/svs';
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="single_block_content"]');
            $title = $content->filterXPath('//h1[@itemprop="headline"]')->text();
            $date = $this->getDate($content->filterXPath('//div[@class="post-meta__date"]/time')->text());
            $image = null;
            ...
            $description = '';
            $descriptionSrc = $content->filterXPath('//div[@itemprop="articleBody"]')->children();
            ...
            $newContentCrawler = $content->filterXPath('//div[@itemprop="articleBody"]')->children();
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@class="container-fluid"]')->filterXPath('//a');
