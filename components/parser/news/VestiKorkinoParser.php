...
class VestiKorkinoParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://vesti-korkino.ru";

    const FEED_SRC = "/news/atom.xml";
    const LIMIT = 100;
    ...
    private static function inflatePost(Crawler $postData): NewsPost
    {
        ...
        $original = $postData->filter("id")->text();


        $createDate = new DateTime($postData->filter("updated")->text());
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("li#content div.news_block_details div.frame img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl($picHolder->attr("src"));
        }

        $descHolder = $crawler->filter("li#content div.news_block_details ul.heading li.name");
        if ($descHolder->count() !== 0) {
            $post->description = Helper::prepareString($descHolder->text());
        }


        $body = $crawler->filter("li#content div.news_block_details div.text");
