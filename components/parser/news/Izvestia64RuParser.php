...
class Izvestia64RuParser implements ParserInterface
{
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," text ")]');

        // $imageNode = $newsPageCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," featured-img ")]');
        // if ($this->crawlerHasNodes($imageNode)) {
        //     $src = $imageNode->attr('data-src');
