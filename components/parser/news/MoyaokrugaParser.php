...
/**
 * News parser from site https://moyaokruga.ru/svbok/
 * @author jcshow
 */
class MoyaokrugaParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://moyaokruga.ru/svbok';
    ...
    public static function getNewsData(): array
    {
        ...
        /** Get news list */
        $curl = Helper::getCurl();
        $newsList = $curl->get(static::SITE_URL . "/?topcategoryid=1");
        if (! $newsList) {
        ...
        $news = $newsListCrawler->filterXPath('//div[contains(@class, "news-container")]//article');
    ...
    public static function getPostDetail(DOMElement $item): NewsPost
    {
        ...
        //Get title block
        $titleBlock = $itemCrawler->filterXPath('//h3//a')->getNode(0);
        ...
        /** Get item datetime */
        $createdAtBlock = $itemCrawler->filterXPath('//div[contains(@class, "txt-articles")]//time')->getNode(0);
        $createdAt = new DateTime($createdAtBlock->textContent . ' ' . date('H:i:s'), new DateTimeZone('Europe/Moscow'));
        $createdAt->setTimezone(new DateTimeZone('UTC'));
        $createdAt = $createdAt->format('c');

        /** Get description */
        $description = self::cleanText($itemCrawler->filterXPath('//div[contains(@class, "txt-articles")]//p[contains(@id, "_articleText")]')->text());

        //Get image if exists
        $picture = null;
        $imageBlock = $itemCrawler->filterXPath('//img[1]')->getNode(0);
        ...
        self::removeNodes($crawler, '//comment() | //br | //hr | //script | //table | //style');
        self::removeNodes($crawler, '//div[@id="content"]//div[contains(@align, "center")]');
        self::removeNodes($crawler, '//div[@id="content"]//div[contains(@class, "top-nav")]');
        self::removeNodes($crawler, '//div[@id="content"]//section[contains(@class, "articles-main")]');
        self::removeNodes($crawler, '//div[@id="content"]//p[contains(@class, "all-photo")]');
        self::removeNodes($crawler, '//div[@id="content"]//div[contains(@id, "_InsidePlaceHolder_divAllImages")]/following-sibling::*');

        $textContent = $crawler->filterXPath('//div[contains(@id,"_articleText")]')->getNode(0);

        // parse description for texts
        foreach ($textContent->childNodes as $node) {
            self::parseNode($post, $node);
        }

        $imgContent = $crawler->filterXPath('//p[contains(@id,"_InsidePlaceHolder_allImages")]')->getNode(0);

        // parse description for texts
        if (! empty($imgContent)) {
            foreach ($imgContent->childNodes as $node) {
                self::parseNode($post, $node);
            }
        }
