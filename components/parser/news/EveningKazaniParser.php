...
class EveningKazaniParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://www.evening-kazan.ru";

    const FEED_SRC = "/rss_news.xml";
    const LIMIT = 100;
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div#content div.node div.content > p");
        ...
        foreach ($body->getNode(0)->childNodes as $bodyNode) {
            ...
            if ($bodyNode->nodeName === "#text" && !empty(trim($bodyNode->textContent, "\xC2\xA0"))) {
            ...
            if ($node->matches("a") && !empty(trim($node->text(), "\xC2\xA0"))) {
                if ($post->description === self::EMPTY_DESCRIPTION) {
                    $post->description = Helper::prepareString($bodyNode->textContent);
                } else {
                    self::addText($post, Helper::prepareString($bodyNode->textContent));
