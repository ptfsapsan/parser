...
class DayOrgRuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://day.org.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $maxNewsCount) {
            ...
            if (!$uri) {
                $uriHtml = mb_substr($newsPreview->html(), mb_strpos($newsPreview->html(), '<link>') + 6, mb_strlen($newsPreview->html()));
                $uriWithoutCheck = $this->encodeUri(mb_substr($uriHtml, 0, mb_strpos($uriHtml, PHP_EOL)));
            ...
            $preview = $this->trimNormalize($newsPreview->filterXPath('//description')->text());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('#content article');
        ...
        $this->removeDomNodes($contentCrawler, '//a[contains(@href,"//tx.me")]');
        $this->removeDomNodes($contentCrawler, '//div[@id="gallery"]');
