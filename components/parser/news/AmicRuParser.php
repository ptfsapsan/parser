...
class AmicRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://www.amic.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $header = $crawler->filter("div.all_news div.pretext");
        if ($header->count() !== 0) {
            $post->description = Helper::prepareString($header->text());
        }

        $body = $crawler->filter("div.all_news div#news_body");
        if ($body->children("p")->count() === 0) {
            $body = $body->filter("div.content");
        }
        if ($body->count() === 0) {
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("div.insider.news")) {
                continue;
            }
