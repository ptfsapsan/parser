...
class Snegir86RuParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('.news__wrap');

        $image = null;
        $mainImageCrawler = $contentCrawler->filterXPath('//div[contains(@class,"news-view__img")]//img[1]');
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($contentCrawler, '//div[contains(@class,"news-view__img")]');
        }
        ...
        $contentCrawler = $contentCrawler->filter('.news-view__text');
