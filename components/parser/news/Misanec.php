 ...
 *
 */
class Misanec extends Aleks007smolBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const BODY_CONTAINER_CSS_SELECTOR = '.content_block';
     ...
     */
    const FEED_URL = 'https://misanec.ru/feed/';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('rss channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function ($node) use (&$curl, &$posts) {
            ...
            $newPost = new NewsPost(
                ...
                str_replace('[&#8230;]', '...', $node->filter('description')->text()) ?: 'description',
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $mainImage = (new Crawler($newsContent))->filter('[loading="lazy"]');
