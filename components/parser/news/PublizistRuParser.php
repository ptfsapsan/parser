...
class PublizistRuParser implements ParserInterface
{
    use \app\components\helper\metallizzer\Cacheable;

    const USER_ID  = 2;
    const FEED_ID  = 2;
    const SITE_URL = 'https://publizist.ru';

    protected static $posts = [];

    public static function run(): array
    {
        $url  = self::SITE_URL.'/rss/rss.xml';
    ...
    protected static function loadPost($item)
    {
        ...
        self::$posts[] = (new Parser())->fill(
            $post,
            $page->filterXpath('//div[@class="blog_entrylist_entry_body"]/node()')
        );
    }
