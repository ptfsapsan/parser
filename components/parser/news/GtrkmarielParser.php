...
class GtrkmarielParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'http://www.gtrkmariel.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/news/news-list/?PAGEN_1={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                ...
                $uri = UriResolver::resolve($titleCrawler->attr('href'), "{$this->getSiteUri()}/news/news-list/");


                $publishedAtString = $newsPreview->filterXPath('//time')->attr('datetime');
                $publishedAt = DateTimeImmutable::createFromFormat('Y-m-d H:i', $publishedAtString,
    ...
    private function searchHeadingNewsItem(DOMNode $node): ?NewsPostItem
    {
        ...
        $newsPostItem = new NewsPostItem(NewsPostItem::TYPE_HEADER, $this->normalizeSpaces($node->textContent), null,
            null, $headingLevel);
        $this->nodeStorage->attach($node, $newsPostItem);
        $this->removeParentsFromStorage($node->parentNode);
