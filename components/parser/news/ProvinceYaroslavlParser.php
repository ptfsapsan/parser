...
class ProvinceYaroslavlParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://www.province.ru";

    const FEED_SRC = "/yaroslavl/component/obrss/yaroslavl-provintsiya-ru.feed";
    const LIMIT = 100;
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("div#k2Container div.itemImageBlock img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl(self::ROOT_SRC . $picHolder->attr("src"));
        }

        $descHolder = $crawler->filter("div#k2Container div#itemIntroText");
        if ($descHolder->count() !== 0 and !empty(trim($descHolder->text(), "\xC2\xA0"))) {
            $post->description = Helper::prepareString($descHolder->text());
        }

        $body = $crawler->filter("div#k2Container div#itemFullText");
