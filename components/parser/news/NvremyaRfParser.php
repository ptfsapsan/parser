...
class NvremyaRfParser implements ParserInterface
{
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/news/?PAGEN_1={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler = $previewNewsCrawler->filter('#page-body td > .news-list .news-item');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $title = $this->trimNormalize($newsPreview->filter('.news-text a')->text());
                $uri = UriResolver::resolve($newsPreview->filter('.news-text a')->attr('href'), $this->getSiteUri());

                $preview = null;
                $previewCrawler = $newsPreview->filter('.nv-list-news-name');
                if ($this->crawlerHasNodes($previewCrawler)) {
                    $preview = $this->trimNormalize($previewCrawler->text());
                }
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('.news-detail');

        $mainImageCrawler = $contentCrawler->filterXPath('//div[contains(@class,"news-picture")]/img[1]');
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($contentCrawler, '//div[contains(@class,"news-detail")]/div[contains(@class,"news-picture")]/img[1]');
        }
        ...
        $contentCrawler = $contentCrawler->filterXPath('//div[contains(@class,"news-text")][1]');

        $this->removeDomNodes($contentCrawler, '//script | //video | //style | //form | //table');
        $this->removeDomNodes($contentCrawler, '//span[1] | //div[contains(@class,"blockquote")][1]');
    ...
    private function getDescriptionFromText(Crawler $crawler): string
    {
        ...
        $text = $this->trimNormalize(strip_tags($crawler->html(), '<br>'));
        $textMatch = array_values(array_filter(array_map('trim', explode('<br>', $text))));
        if (count($textMatch)) {
            $description = $textMatch[0];
        }
    ...
    private function getPublishedAt(Crawler $crawler): ?DateTimeImmutable
    {
        ...
        $publishedAtString = Text::trim($crawler->filter('.news-date-time')->text());
