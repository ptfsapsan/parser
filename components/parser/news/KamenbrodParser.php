...
class KamenbrodParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://kamenbrod.issa.pnzreg.ru";
    ...
    public static function run(): array
    {
        ...
        $items = $document->childNodes->item(0)->childNodes->item(1)->childNodes;
        if ($items->count() === 0) {
     ...
     *
     * @param DOMNode $postData
     *
     * @return NewsPost
     * @throws Exception
     */
    private static function inflatePost(DOMNode $postData): NewsPost
    {


        $title = $postData->childNodes->item(1)->nodeValue;

        $original = $postData->childNodes->item(3)->nodeValue;

        $nodeIndex = $postData->childNodes->count() - 2;
        $createDate = new DateTime($postData->childNodes->item($nodeIndex)->nodeValue);
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("div.l-news div.l-news__photo img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl(self::ROOT_SRC . $picHolder->attr("src"));
        }

        $body = $crawler->filter("div.l-news div.l-news__text div.l-news__text-content");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if (
                $node->matches("p")
                && $node->children("br")->count() !== 0
                && !empty(trim($node->text(), "\xC2\xA0"))
            ) {

                foreach ($node->getNode(0)->childNodes as $inNode) {
                    if ($inNode->nodeName === "#text" && !empty(trim($inNode->nodeValue, " \n\r\xC2\xA0"))) {
                        if (empty($post->description)) {
                            $post->description = Helper::prepareString(trim($inNode->nodeValue));
                        } else {
                            self::addText($post, trim($inNode->nodeValue));
                        }
                    }
                }
