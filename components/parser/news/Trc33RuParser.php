...
/**
 * Парсер новостей с сайта http://trc33.ru/
 */
class Trc33RuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'http://trc33.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="news-pholder"]');
            $title = $content->filterXPath('//h2[@class="news-detail-header"]')->text();
            $date = $this->getDate($itemCrawler->filterXPath('//td[@class="page-area"]')->filterXPath('//span[@class="date-active"]')->text());
            $image = null;
            $imgSrc = $content->filterXPath('//img');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }
            $description = $content->filterXPath('//p[1]')->text();
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//td[@class="page-area"]')->filterXPath('//a[@class="news-anons-box-right"]');
