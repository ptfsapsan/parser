...
class VoshodParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://voshod.rbsmi.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div.page div.container div.pin");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("p") && !empty(trim($node->text(), "\xC2\xA0"))) {
                /** @var DOMNode $pDomNode */
                foreach ($node->getNode(0)->childNodes as $pDomNode) {
                    if (!empty(trim($pDomNode->textContent))) {
                        if (empty($post->description)) {
                            $post->description = Helper::prepareString(trim($pDomNode->textContent));
                        } else {
                            self::addText($post, trim($pDomNode->textContent));
                        }
                    }
                }
