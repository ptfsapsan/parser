...
class PrimerruParser implements ParserInterface
{
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"fulltext")]');
        ...
        $this->removeDomNodes($contentCrawler, '//div[@class="list-inline-item"]');
        $this->removeDomNodes($contentCrawler, '//a[1]');
        $this->removeDomNodes($contentCrawler, '//h1[@class="card-title"]');
        $this->removeDomNodes($contentCrawler, '//div[@class="it-img"]');
        $this->removeDomNodes($contentCrawler, '//p[@class="card-text"]//following-sibling::div[1]');
