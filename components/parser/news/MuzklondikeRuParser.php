...
class MuzklondikeRuParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('#main_content .text');

        $descriptionCrawler = $contentCrawler->filterXPath('//i[1]');
        if ($this->crawlerHasNodes($descriptionCrawler)) {
            $description = Text::trim($this->normalizeSpaces($descriptionCrawler->text()));
            $this->removeDomNodes($contentCrawler, '//i[1]');
        }

        $contentCrawler = $contentCrawler->filter('.h_img');
