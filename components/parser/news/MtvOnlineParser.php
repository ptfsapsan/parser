...
class MtvOnlineParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://xn--b1ats.xn--80asehdb';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
            ...
            $uri = UriResolver::resolve($uriCrawler->text(), $this->getSiteUri());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//article[@itemtype="http://schema.org/NewsArticle"]');

        $mainImageCrawler = $newsPageCrawler->filterXPath('//meta[@property="twitter:image:src"]')->first();
        ...
        $contentCrawler = $newsPostCrawler->filterXPath('//p[@itemprop="description"]/following-sibling::*[not(contains(@class,"item-foot"))]');
