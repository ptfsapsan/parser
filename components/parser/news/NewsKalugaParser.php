...
/**
 * News parser from site https://www.newskaluga.ru/
 * @author jcshow
 */
class NewsKalugaParser implements ParserInterface
{
    ...
    public const SITE_URL = 'https://www.newskaluga.ru';
    ...
    public static function getNewsData(int $limit = 100): array
    {
        ...
        while (self::$parsedCount < $limit) {
            /** Get RSS news list */
            $curl = Helper::getCurl();
            $newsList = $curl->get(static::SITE_URL . "/column/?pgnum=$pageNum");
            if (! $newsList) {
            ...
            $news = $newsListCrawler->filterXPath('//div[contains(@class, "news-list")]//div[contains(@class, "news-rub-list-item")]');
    ...
    public static function getPostDetail(DOMElement $item): NewsPost
    {
        ...
        //Get title block
        $titleBlock = $itemCrawler->filterXPath('//a[contains(@class, "nrli-title")]')->getNode(0);

        /** Get item detail link */
        $link = UriResolver::resolve(self::cleanUrl($titleBlock->getAttribute('href')), static::SITE_URL);

        /** Get title */
        $title = self::cleanText($titleBlock->textContent);

        /** Get item datetime */
        $createdAtBlock = $itemCrawler->filterXPath('//div[contains(@class, "news_info")]//span[1]')->getNode(0);
        $createdAt = new DateTime($createdAtBlock->textContent, new DateTimeZone('Europe/Moscow'));
        $createdAt->setTimezone(new DateTimeZone('UTC'));
        $createdAt = $createdAt->format('c');

        /** Get description */
        $description = self::cleanText($itemCrawler->filterXPath('//div[contains(@class, "nrli-descr")]')->text());

        //Get image if exists
        $picture = null;
        $imageBlock = $itemCrawler->filterXPath('//div[contains(@class, "nrli-img")]//a[1]')->getNode(0);
        if (! empty($imageBlock) === true) {
            $picture = $imageBlock->getAttribute('style');
            $picture = preg_replace('/(.*)(url\(\')([^\']*)(\'.*)/', '$3', $picture);
            $picture = UriResolver::resolve(self::cleanUrl($picture), static::SITE_URL);
        }
        ...
        self::removeNodes($crawler, '//div[contains(@class, "news-view-author-newsinfo")]');

        $detailPage = $crawler->filterXPath('//div[contains(@class, "js-mediator-article")]')->getNode(0);
