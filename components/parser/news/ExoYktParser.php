...
class ExoYktParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://www.exo-ykt.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $url = "/news/25/rss/?PAGEN_1={$pageNumber}";
            ...
            $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[@class="article-01-item"]');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList,$url ) {
                $titleCrawler = $newsPreview->filterXPath('//span[contains(@class,"article-01-item-name")]/a');
                ...
                $publishedAtString = $newsPreview->filterXPath('//span[contains(@class,"article-01-item-date")]')->text();
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@class="detail-block"]');

        $mainImageCrawler = $newsPageCrawler->filterXPath('//div[@class="detail-block-img"]/a')->first();
        ...
        $description = $newsPostCrawler->filterXPath('//div[contains(@class,"detail-block-preview-text")]')->text();
        if (!$description) {
            $description = self::EMPTY_DESCRIPTION_KEY;
        }

        $contentXPath = '//div[@class="detail-block-detail-text"]';
