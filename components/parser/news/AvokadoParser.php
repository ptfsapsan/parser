...
class AvokadoParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    private const LINK = 'https://avoka.do/feed';
    private const DOMAIN = 'https://avoka.do';
    ...
    public static function run(): array
    {
        ...
        if (count($items)) {
            ...
            foreach ($items as $item) {
                ...
                $image = $originalParser->find('[data-for=reactionLine]')->next()->find('img')->attr('src');
    ...
    {
        $paragraphs = $parser->find('.post-body-text p');
        if (count($paragraphs)) {
