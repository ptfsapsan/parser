...
class VecerneaiaReazaniParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://www.vr-gazeta.ru";
    ...
    public static function run(): array
    {
        ...
        for ($pageId = 1; $pageId <= ceil(self::LIMIT / self::NEWS_PER_PAGE); $pageId++) {
            ...
            $items = $crawler->filter("div.blog-item");
    ...
    {
        $title = $postData->filter("div.desc div.name a")->text();

        $original = self::normalizeUrl(self::ROOT_SRC . $postData->filter("div.desc div.name a")->attr("href"));


        $dateHolder = $postData->filter("div.desc div.date");
        ...
        $dateString = $dateArr[2] . "-" . self::MONTHS[$dateArr[1]] . "-" . $dateArr[0] . " " . $dateArr[3] . " +03:00";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("div.white-box.v-2.inner-page div.news-image img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl(self::ROOT_SRC . $picHolder->attr("src"));
        }

        $body = $crawler->filter("div.white-box.v-2.inner-page div.news-text");
        ...
        foreach ($body->getNode(0)->childNodes as $bodyNode) {
            ...
            if ($node->matches("div") && !empty(trim($node->text(), "\xC2\xA0"))) {
                foreach ($node->getNode(0)->childNodes as $pNode) {
                    if (empty(trim($pNode->textContent))) {
                        continue;
                    }
                    if (empty($post->description)) {
                        $post->description = Helper::prepareString($pNode->textContent);
                    } else {
                        self::addText($post, Helper::prepareString($pNode->textContent));
                    }
                }
