...
class VzNnRuParser implements ParserInterface
{
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        try {
            ...
            $previewNewsContent = html_entity_decode($previewNewsContent);
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@class="news-detail"]//div[@class="news-item"]');

        $imageNode = $newsPostCrawler->filterXPath('//div[@class="info"]//div//img');
        if ($this->crawlerHasNodes($imageNode)) {
            $src = $imageNode->attr('src');
            $image = $src ? UriResolver::resolve($src, $this->getSiteUri()) : null;
        }

        $this->removeDomNodes($newsPostCrawler, '//text()[contains(., "Фото:")]
        | //*[@class="vdate"]
        | //*[@class="stat"]');
