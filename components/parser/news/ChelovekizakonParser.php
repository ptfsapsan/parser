...
class ChelovekizakonParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://chelovekizakon.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount){
            ...
            $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[@class="row justify-content-center mt-5"]');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $titleCrawler = $newsPreview->filterXPath('//h3/a');
                $title = $titleCrawler->text();
                $uri = $titleCrawler->attr('href');
                $uri = $this->encodeUri($uri);

                $timezone = new DateTimeZone('Europe/Moscow');
                $publishedAtString = $newsPreview->filterXPath('//div[@class="col-md-6 text-right"]')->text();
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $mainImageCrawler = $newsPageCrawler->filterXPath('//img[@itemprop="image"]')->first();
        ...
        $description = $newsPostCrawler->filterXPath('//div[@itemprop="description"]')->text();
        if(!$description){
            $description = self::EMPTY_DESCRIPTION_KEY;
        }

        $contentXPath = '//div[@itemprop="articleBody"]';
