...
/**
 * Парсер новостей с сайта http://www.vpered-kr.ru/
 */
class VperedKrRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'http://www.vpered-kr.ru';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $title = $article->filterXPath("//h2[@itemprop='name']")->text();
            $date = $this->getDate($article->filterXPath("//time")->attr('datetime'));
            $image = null;
            $imgSrc = $article->filterXPath("//img[@class='media-photos_img']");
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }

            $description = $this->clearText($article->filterXPath('//div[@itemprop="articleBody"]')->text());
            if (!$description) {
                $description = $title;
            }

            if (strpos($description, 'document.getElementById') !== false) {
                continue;
            }
            ...
            $newContentCrawler = $itemCrawler->filterXPath('//div[@class="media-block media-text __full"]');
            if ($newContentCrawler->getNode(0)) {
                $newContentCrawler = $newContentCrawler->children();
            ...
            $newImgCrawler = $itemCrawler->filterXPath('//img[@class="media-photos_img"]');
            foreach ($newImgCrawler as $content) {
                if ($content->getAttribute('src') != $image) {

                    $this->addItemPost($post, NewsPostItem::TYPE_IMAGE, null, $content->getAttribute('src'));
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath("//div[@class='blog-featured']")->filterXPath('//h2/a[@itemprop="url"]');
