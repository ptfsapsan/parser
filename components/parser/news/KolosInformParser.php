...
class KolosInformParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    private const LINK = 'https://kolosinform.ru/feed/';
    private const DOMAIN = 'https://kolosinform.ru';
    ...
    public static function run(): array
    {
        ...
        if (count($items)) {
            foreach ($items as $item) {
                ...
                $image = $originalParser->find('.post img:first')->attr('src');
    ...
    private static function setOriginalData(PhpQueryObject $parser, NewsPost $post): NewsPost
    {
        ...
        $images = $parser->find('.post img:gt(0)');
        if (count($images)) {
