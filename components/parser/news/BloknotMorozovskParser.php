...
class BloknotMorozovskParser extends MediasferaNewsParser implements ParserInterface
{
    ...
    public const SITE_URL = 'http://bloknot-morozovsk.ru/';
    public const NEWSLIST_URL = 'http://bloknot-morozovsk.ru/rss_news.php';
    ...
    public static function run(): array
    {
        ...
        $listCrawler->filterXPath(self::NEWSLIST_POST)->slice(0, self::NEWS_LIMIT)->each(function (Crawler $node) use (&$posts) {
            ...
            foreach ($newsPost->items as $key => $item) {
                ...
                elseif ($item->type == NewsPostItem::TYPE_IMAGE) {
                    if(strpos($item->image, 'kontakty.jpg')) {
                        unset($newsPost->items[$key]);
                        $removeNext = true;
                    }
                }
