...
/**
 * Парсер новостей с сайта https://lentv24.ru/
 */
class LenTv24RuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://lentv24.ru/';
    ...
    public function parse(): array
    {
        ...
        $urlNews = self::SITE_URL . '/Novosti.htm';
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $title = $itemCrawler->filterXPath('//header[@class="title"]/h1')->text();
            $date = $this->getDate($itemCrawler->filterXPath('//div[@class="meta-item meta-item__creation-date"]')->text());
            $image = null;
            $imgSrc = $itemCrawler->filterXPath('//div[@class="photo"]/img[@class="main-image"]');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }
            $description = $itemCrawler->filterXPath("//div[@class='inner']/p")->text();
            ...
            $newContentCrawler = $itemCrawler->filterXPath("//div[@class='text-block']")->children();
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//article[@class="announce"]')->filterXPath('//header[@class="title"]/a');
