...
class RusFastParser implements ParserInterface
{
    ...
    public static function run(): array
    {
        ...
        if (count($items)) {
            foreach ($items as $item) {
                ...
                $image = $originalParser->find('.shot-text2 img')->attr('src');
    ...
    {
        $text = $parser->find('.shot-text2 ~:not(script)')->text();
        if (!empty($text)) {
            $post->addItem(
                new NewsPostItem(
        ...
        $images = $parser->find('.shot-text2 img');
        if (count($images)) {
        ...
        $links = $parser->find('.shot-text2 a');
        if (count($links)) {
