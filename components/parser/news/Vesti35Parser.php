...
class Vesti35Parser implements ParserInterface
{
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $vestiCrawler = $newsPageCrawler->filterXPath('//*[contains(@class,"vesti_page")]');
        $tvProgramCrawler = $newsPageCrawler->filterXPath('//*[contains(@class,"tvprogram_page")]');

        if ($this->crawlerHasNodes($vestiCrawler)) {
            $titleCrawler = $vestiCrawler->filterXPath('//div[contains(@class,"vesti_page_header")]/h1')->first();
        ...
        if (!$description) {
            $description = $newsPageCrawler->filterXPath('//meta[@name="description"]')->attr('content');
        }
