...
/**
 * News parser from site https://www.4vsar.ru/
 * @author jcshow
 */
class ChetvertayaVlastParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://www.4vsar.ru';
    ...
    public static function getNewsData(): array
    {
        ...
        /** Get RSS news list */
        $curl = Helper::getCurl();
        $newsList = $curl->get(static::SITE_URL . "/rss/index.php");
        if (! $newsList) {
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        $description = strip_tags(preg_replace('/(<a.*\/a>)/', '', $description));
        ...
        //Get image if exists
        $picture = null;
        $imageBlock = $crawler->filterXPath('//div[contains(@class, "newsWrapper")]//img[1]')->getNode(0);
        ...
        self::removeNodes($crawler, '//div[contains(@class, "newsWrapper")]//h1[contains(@class, "mh1")]');
        self::removeNodes($crawler, '//div[contains(@class, "newsWrapper")]//p[contains(@class, "date")]');
        self::removeNodes($crawler, '//div[contains(@class, "newsWrapper")]//div[contains(@class, "share")]');
        self::removeNodes($crawler, '//div[contains(@class, "newsWrapper")]//div[@id="text"]/following-sibling::*');

        $detailPage = $crawler->filterXpath('//div[contains(@class, "newsWrapper")]')->getNode(0);
