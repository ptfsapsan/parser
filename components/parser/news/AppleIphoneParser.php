...
class AppleIphoneParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://www.apple-iphone.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("article div.entry-inner");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("p") && $node->filter("script")->count() !== 0) {
                continue;
            }
