...
class RegionsRuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://regions.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/section/main/newslist?p={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler = $previewNewsCrawler->filter('#main .inner-block > .post');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $title = Text::trim($this->normalizeSpaces($newsPreview->filter('.title-post a')->text()));
                $uri = UriResolver::resolve($newsPreview->filter('.title-post a')->attr('href'), $this->getSiteUri());

                $publishedAtString = $newsPreview->filter('.date-post')->text();
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filterXPath('//div[@class="post"]//div[@class="content-entry"] | //div[@class="post"]//div[@class="comments-entry"]');

        $description = $this->getDescriptionFromText($contentCrawler);

        $mainImageCrawler = $newsPageCrawler->filter('.main-gal-img img')->first();
    ...
    private function getDescriptionFromText(Crawler $crawler): string
    {
        ...
        if (!$description) {
            $description = $crawler->filterXPath('//div[1]//div[1]')->text();
            $this->removeDomNodes($crawler, '//div[1]//div[1]');
        }
