...
class InVidnoeRuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://in-vidnoe.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/novosti?page={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler = $previewNewsCrawler->filter('.list-yii-wrapper > .list3 > .news-itm');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $title = Text::trim($this->normalizeSpaces($newsPreview->filter('.news-itm__title a')->text()));
                $uri = UriResolver::resolve($newsPreview->filter('.news-itm__title a')->attr('href'), $this->getSiteUri());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('.b-page .b-page__main');

        $mainImageCrawler = $newsPageCrawler->filter('.b-page__image img')->first();
        ...
        $contentCrawler = $contentCrawler->filter('.b-page__content');
        $this->removeDomNodes($contentCrawler, '//p[contains(@class,"print")]');
    ...
    private function getPublishedAt(Crawler $crawler): DateTimeImmutable
    {
        ...
        $publishedAt = Text::trim($crawler->filter('.news-itm__date')->text());
        $publishedAtString = str_replace([...$months, ' г.,'], [...array_keys($months), ''], $publishedAt);
