...
class UgraTvParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://ugra-tv.ru";
    ...
    public static function run(): array
    {
        ...
        $items = $crawler->filter("div.news__item");
        if ($items->count() === 0) {
    ...
    {
        $title = $postData->filter("div.news-regular__content a.news-regular__title")->text();

        $original = self::normalizeUrl(self::ROOT_SRC . $postData->filter("div.news-regular__content a.news-regular__title")->attr("href"));

        $dateString = $postData->filter("span.misc-info__date")->text();

        if (mb_stripos($dateString, "назад") !== false) {
            $createDate = new DateTime('now', new DateTimeZone("+05:00"));
            $dateArr = explode(" ", $dateString);
            $createDate->modify("-" . $dateArr[0] . " minutes");
        } elseif (mb_stripos($dateString, "сегодня") !== false) {
            $createDate = new DateTime('now', new DateTimeZone("+05:00"));
            $dateArr = explode(",", $dateString);
            $timeArr = explode(":", $dateArr[1]);
            $createDate->setTime($timeArr[0], $timeArr[1]);
        } elseif (mb_stripos($dateString, "вчера") !== false) {
            $createDate = new DateTime('now', new DateTimeZone("+05:00"));
            ...
            $createDate = DateTime::createFromFormat("d.m.Y H:i:s P", $dateString . ":00 +05:00");
            if ($createDate === false) {
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("div.single-page__content  div.single-page__main-image img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl(self::ROOT_SRC . $picHolder->attr("src"));
        }

        $body = $crawler->filter("div.single-page__content  div.single-page__text");
        ...
        foreach ($body->getNode(0)->childNodes as $bodyNode) {
            ...
            if ($node->matches("div.news-quote") && !empty(trim($node->text(), "\xC2\xA0"))) {
                self::addQuote($post, $node->filter("div.news-quote__text")->text());
                continue;
            }
