...
class Zt116Parser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://zt116.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $url = "/news/widget/list/goryachie-novosti/page/{$pageNumber}";
            ...
            $previewNewsCrawler = $previewNewsCrawler->filterXPath('//li[contains(@id,"news")]');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $url) {
                $titleCrawler = $newsPreview->filterXPath('//a[contains(@class,"media-list__head")]');
                $uri = UriResolver::resolve($titleCrawler->attr('href'), $this->getSiteUri() . $url);
                $title = trim($titleCrawler->text());

                $publishedAtUTC = null;

                $preview = trim($newsPreview->filterXPath('//div[contains(@class,"media-list__body")]')->text());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"layout__body")]/div[contains(@class,"page-main")]')->first();
        ...
        $contentCrawler = $newsPostCrawler->filterXPath('//div[contains(@class,"page-main__text")]');

        $this->removeDomNodes($contentCrawler,
            '//div[contains(text(),"Реклама")]/ancestor::*[parent::div[contains(@class,"page-main__text")]]');
