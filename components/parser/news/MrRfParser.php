...
/**
 * News parser from site https://mr-rf.ru/
 * @author jcshow
 */
class MrRfParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://mr-rf.ru';
    ...
    public static function getPostDetail(SimpleXMLElement $item): NewsPost
    {
        ...
        /** Get description */
        $description = strip_tags(self::cleanText($item->description));

        //Get image if exists
        $picture = $item->enclosure;
        if (! empty($picture) === true) {
            $picture = self::cleanUrl($picture->attributes()->url[0]);
        ...
        $detailPage = $crawler->filterXPath('//div[contains(@class,"news_text")]')->getNode(0);
