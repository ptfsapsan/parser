 ...
 *
 */
class MichurinskName extends Aleks007smolBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const MAIN_PAGE_URI = 'https://michurinsk.name';
     ...
     */
    const FEED_URL = 'https://michurinsk.name/feed/';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function ($node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $mainImage = (new Crawler($newsContent))->filter('.entry-image img');
                 ...
                 */
                $annotation = (new Crawler($newsContent))->filter('figcaption.wp-caption-text');
