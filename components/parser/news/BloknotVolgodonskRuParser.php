 ...
 * @rss_html
 */
class BloknotVolgodonskRuParser extends MediasferaNewsParser implements ParserInterface
{
    ...
    public const SITE_URL = 'https://bloknot-volgodonsk.ru/';
    public const NEWSLIST_URL = 'https://bloknot-volgodonsk.ru/rss_news.php';
        ...
        $listCrawler->filterXPath(self::NEWSLIST_POST)->slice(0, self::NEWS_LIMIT)->each(function (Crawler $node) use (&$posts) {
            ...
            foreach ($newsPost->items as $key => $item) {
                ...
                if($item->type == NewsPostItem::TYPE_TEXT) {
                    ...
                    else if(strpos($text, 'Если вам есть, чем поделиться') !== false) {
                        unset($newsPost->items[$key]);
                        $removeNext = true;
                    }
                    else if(strpos($text, 'Работу для всех и каждого') !== false) {
                        unset($newsPost->items[$key]);
                        $removeNext = true;
                    }
