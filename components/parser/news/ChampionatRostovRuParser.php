...
class ChampionatRostovRuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://championat-rostov.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            ...
            $previewNewsCrawler = $previewNewsCrawler->filter('.news > .news__item');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $publishedAt = $this->getPublishedAt($newsPreview);
                $this->removeDomNodes($newsPreview, '//div[contains(@class,"news__item-info")]');

                $title = $this->trimNormalize($newsPreview->filter('h4')->text());
                $uri = UriResolver::resolve($newsPreview->filter('a')->first()->attr('href'), $this->getSiteUri());
                $preview = $this->trimNormalize($newsPreview->filter('a > p')->text());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('.inner-news__article');
        $this->removeDomNodes($contentCrawler, '//h2[1]');

        $mainImageCrawler = $newsPageCrawler->filterXPath('//img[contains(@class,"inner-news__photo")][1]');
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($contentCrawler, '//img[contains(@class,"inner-news__photo")][1]');
        }
        ...
        foreach ($contentCrawler as $item) {
            ...
            foreach ($nodeIterator->getRecursiveIterator() as $k => $node) {
                ...
                if (!$newsPostItem || $newsPostItem->text === $description) {
                    continue;
                }

                if (!count($newsPost->items ?? []) && (str_contains($newsPostItem->text, 'ИА "Чемпионат-Ростов"') || str_contains($newsPostItem->text, 'ИА «Чемпионат-Ростов»'))) {
                    continue;
                }
    ...
    private function getPublishedAt(Crawler $crawler): DateTimeImmutable
    {
        ...
        $publishedAtString = Text::trim($crawler->filter('.item-info__date')->text());
