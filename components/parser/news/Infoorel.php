...
/**
 * Парсер новостей из RSS ленты https://www.infoorel.ru/
 *
 */
class Infoorel extends TyRunBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const MAIN_PAGE_URI = 'https://www.infoorel.ru';

    /**
     * CSS класс, где хранится содержимое новости
     */
    const BODY_CONTAINER_CSS_SELECTOR = 'article.news-view';
     ...
     */
    const CONTENT_CSS_SELECTOR = '.uk-clearfix';

    /**
     * Классы эоементов, которые не нужно парсить, например блоки с рекламой и т.п.
     * в формате RegExp
     */
    const EXCLUDE_CSS_CLASSES_PATTERN = '/news-view-big-img/';
     ...
     */
    const FEED_URL = 'https://www.infoorel.ru/news/rss2.php';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('rss channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function (Crawler $node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $mainImage = $newsContent->filter('figure.news-view-big-img img');
                if ($mainImage->count()) {
                ...
                /**
                 * Если нет описания берем первую текстовую ноду
                 */
