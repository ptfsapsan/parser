...
class RadiuscityRuParser implements ParserInterface
{
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[contains(@id,"ctl00_ContentPlaceHolder1_arangesAll1_arrangeshere")]/a');
        ...
        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $maxNewsCount) {
            ...
            $title = $newsPreview->filterXPath('//div[1]')->text();
            $uri = UriResolver::resolve(Helper::encodeUrl($newsPreview->attr('href')), $this->getSiteUri());
            $preview = Text::trim($this->normalizeSpaces($newsPreview->filterXPath('//div[last()]')->text()));
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            ...
            $this->removeDomNodes($newsPageCrawler, '//div[contains(@id,"ArticleImagesFull")]//img[1]');
        }
