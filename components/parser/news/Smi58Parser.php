...
class Smi58Parser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    private const LINK = 'https://smi58.ru/news/rss/';
    private const DOMAIN = 'https://smi58.ru';
    ...
    public static function run(): array
    {
        ...
        if (count($items)) {
            ...
            foreach ($items as $item) {
                ...
                $image = $originalParser->find('.news-detail_img img')->attr('src');
    ...
    {
        $paragraphs = $parser->find('.news-detail_text p');
        if (count($paragraphs)) {
