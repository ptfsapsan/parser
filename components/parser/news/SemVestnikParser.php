...
class SemVestnikParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://semvestnik.ru";

    const FEED_SRC = "/?format=feed&type=rss";
    const LIMIT = 100;
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $pageData = str_ireplace('<?xml version="1.0" encoding="utf-8"?>', "", $pageData);
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("div.moduletable")) {
                continue;
            }
            ...
            if ($node->matches("div.sigplus-gallery") && $node->filter("img")->count() !== 0) {
                $image = $node->filter("img");
                $image->each(function (Crawler $imgNode) use ($post) {
                    $src = self::ROOT_SRC . self::normalizeUrl($imgNode->attr("src"));
                    if ($post->image === null) {
                        $post->image = $src;
                    } else {
