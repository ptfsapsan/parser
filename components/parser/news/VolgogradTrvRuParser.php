...
class VolgogradTrvRuParser implements ParserInterface
{
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve('/Default.aspx', $this->getSiteUri());
        ...
        $links = $previewNewsCrawler->filterXPath('//*[@class="corners_1"]/a');
        ...
        $links->each(function (Crawler $newsPreview, $i) use (&$previewList, $previewNewsCrawler) {
            ...
            $title = trim($newsPreview->text());
            
            $publishedAtString = trim($previewNewsCrawler->filterXPath('//*[@class="sublinks_1"]/text()')->eq($i)->text());
            $publishedAt = DateTimeImmutable::createFromFormat('d.m.Y H:i', $publishedAtString, new DateTimeZone('Europe/Volgograd'));
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//*[contains(concat(" ",normalize-space(@class)," ")," interval_2 ")]');
