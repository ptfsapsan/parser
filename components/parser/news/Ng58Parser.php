...
class Ng58Parser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'http://ng58.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/news/news/?PAGEN_1={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[contains(@class,"td-ss-main-content")]/div[contains(@class,"td_module_wrap")]');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $title = $newsPreview->filterXPath('//*[contains(@class,"entry-title")]/a')->text();
                $uri = UriResolver::resolve($newsPreview->filterXPath('//*[contains(@class,"entry-title")]/a')->attr('href'), $this->getSiteUri());
                $preview = $newsPreview->filterXPath('//div[contains(@class,"item-details")]//div[contains(@class,"td-excerpt")]');
                $preview = $this->crawlerHasNodes($preview) ? trim(strip_tags($preview->text())) : null;
                $publishedAt = $newsPreview->filterXPath('//time')->text();
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $mainImageCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"detail-img-data")]')->first();
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('data-detimg');
        }
        ...
        $contentCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"td-post-content")]');

        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"td-post-content")]/span[contains(@class,"quote-lid")]');
        ...
        $this->removeDomNodes($contentCrawler, '//div[contains(@id,"comments")]');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"news-detail-back")]');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"td_block_baner")]');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"td-post-sharing")]');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"td-login-info-text")]');

        if (!$description) {
            $description = $contentCrawler->filterXPath('//div')->text();
        }
        ...
        foreach ($newsPost->items as $key => $item) {
            if ($item->type !== NewsPostItem::TYPE_IMAGE && mb_strlen($autoGeneratedDescription) < 200) {
                if ($item->text) {
                    $space = $autoGeneratedDescription === '' ? '' : ' ';
                    $autoGeneratedDescription .= $space . $item->text;
