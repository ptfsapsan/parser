...
/**
 * Парсер новостей с сайта https://svgz.ru/
 */
class SvgzRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://svgz.ru';
    ...
    public function parse(): array
    {
        ...
        $urlNews = self::SITE_URL . '/vse-novosti-vyksa/';
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="entry-content-wrap read-single"]');
            $title = $content->filterXPath('//h1[@class="entry-title"]')->text();
            $date = $this->getDate($content->filterXPath('//span[@class="item-metadata posts-date"]')->text());
            $image = null;
            $imgSrc = $content->filterXPath('//div[@class="read-img pos-rel"]')->filterXPath('//img');
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }
            $description = '';
            $descriptionSrc = $content->filterXPath('//div[@class="color-pad"]')->children();
            ...
            $newContentCrawler = $content->filterXPath('//div[@class="color-pad"]')->children();
            foreach ($newContentCrawler as $contentNew) {
                if ($contentNew->childNodes->count() > 1) {
                    foreach ($contentNew->childNodes as $childNode) {
                        $classes = ['block-latest-posts', 'Sss', '_sss_', 'post-item-metadata', 'navigation'];
                        $next = false;
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath('//div[@class="elementor-widget-container"]')->filterXPath('//a[@class="eael-grid-post-link"]');
