...
class Logos44RuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://logos44.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve('/novosti.html', $this->getSiteUri());
        ...
        $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," itemContainer ")]');

        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
            $anchorLink = $newsPreview->filterXPath('//*[contains(concat(" ",normalize-space(@class)," ")," catItemTitle ")]//a');
            $title = $anchorLink->text();
            $uri = UriResolver::resolve($anchorLink->attr('href'), $this->getSiteUri());
            ...
            $preview = $newsPreview->filterXPath('//*[contains(concat(" ",normalize-space(@class)," ")," catItemIntroText ")]')->text();
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        try {
            $json = $newsPageCrawler->filterXPath('//script[@type="application/ld+json"]')->text();
            $object = json_decode($json);
            $publishedAtString = $object->datePublished;
            ...
            die();
            $publishedAtString = $newsPageCrawler->filterXPath('//*[@class="itemDateCreated"]');
        ...
        $this->removeDomNodes($newsPostCrawler, '//*[contains(text(), "Фото:")]
        | //*[contains(concat(" ",normalize-space(@class)," ")," itemDateCreated ")]
        | //*[@class="itemIntroText"]//p[1][descendant::*[contains(text(), "Logos44")]]');
    ...
    private function searchLinkNewsItem(DOMNode $node, PreviewNewsDTO $previewNewsItem): ?NewsPostItem
    {
        ...
        if (!$node instanceof DOMElement || !$this->isLink($node) || str_contains($node->getAttribute('href'), 'XL')) {
            return null;
        }
