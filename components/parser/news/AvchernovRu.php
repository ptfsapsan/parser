 ...
 *
 */
class AvchernovRu extends Aleks007smolBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const MAIN_PAGE_URI = 'https://avchernov.ru';

    /**
     * CSS класс, где хранится содержимое новости
     */
    const BODY_CONTAINER_CSS_SELECTOR = '.entry-the-content';
     ...
     */
    const FEED_URL = 'https://avchernov.ru/feed/';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function ($node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $mainImage = (new Crawler($newsContent))->filter('[itemprop="image"] img');
    ...
    private static function parseParagraph(Crawler $node, NewsPost $newPost, array $descriptionSentences): void
    {
        ...
        if ($newPost->description == 'description' || $newPost->description == '') {
            if (!empty($nodeSentences)) {
                $description = self::prepareDescription(implode('. ', $nodeSentences));
