 ...
 *
 */
class Up74 extends Aleks007smolBaseParser implements ParserInterface
{
    /*run*/
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const MAIN_PAGE_URI = 'https://up74.ru/rss';

    /**
     * CSS класс, где хранится содержимое новости
     */
    const BODY_CONTAINER_CSS_SELECTOR = '.detail-text';
     ...
     */
    const FEED_URL = 'https://up74.ru/rss/';
    ...
    public static function run(): array
    {
        ...
        /**
         * Ломается структура XML, если в ней присутствует тег <link>, при замене на <reallink> проблем с парсингом ссылки на новость нет
         */
        $rss = str_replace(['<link>', '</link>'], ['<reallink>', '</reallink>'], $rss);
    ...
    {
        /**
         * Парсинг доп. блока текста
         */
        if($node->attr('class') === 'box-inner-block') {
...
//                }
    ...
    protected static function parseLink(Crawler $node, NewsPost $newPost): void
    {
        if (filter_var($node->attr('href'), FILTER_VALIDATE_URL)
            ...
            && strpos($node->attr('href'), '.jpg') === false) {
            $newPost->addItem(
                new NewsPostItem(
