...
/**
 * Парсер новостей с сайта https://evo-rus.com/
 */
class EvoRusComParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://evo-rus.com';
    ...
    public function parse(): array
    {
        ...
        $urlNews = self::SITE_URL . '/category/auto-news';
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="wpb_wrapper"]');
            $title = $content->filterXPath("//h1[@class='tdb-title-text']")->text();
            $date = $this->getDate($content->filterXPath("//time[@class='entry-date updated td-module-date']")->attr('datetime'));
            $image = null;
            $imgSrc = $content->filterXPath("//div[@class='tdb-block-inner td-fix-index']/img");
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('data-src'));
            }

            $description = $content->filterXPath("//div[@class='single-bloquote']")->text();
            ...
            $newContentCrawler = $content->filterXPath("//div[@class='single-bloquote']")->nextAll();
            foreach ($newContentCrawler as $key=>$contentNew) {
                if ($key == 0 && $contentNew->nodeName == 'div') {
                    continue;
                }
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath("//div[@class='wpb_wrapper']")->filterXPath("//h3[@class='entry-title td-module-title']/a");
