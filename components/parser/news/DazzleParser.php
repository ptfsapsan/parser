...
class DazzleParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    private const LINK = 'https://dazzle.ru/rss.xml';
    ...
    public static function run(): array
    {
        ...
        if (count($items)) {
            ...
            foreach ($items as $item) {
                ...
                $image = $originalParser->find('.full-news-text > img')->attr('src');
    ...
    {
        $paragraphs = $parser->find('.full-news-text p');
        if (count($paragraphs)) {
