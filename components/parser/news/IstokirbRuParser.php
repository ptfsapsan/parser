...
class IstokirbRuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://istokirb.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            ...
            $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[contains(@class,"main-item")]/div[contains(@class,"block-maim-item-res")]');
            ...
            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $maxNewsCount) {
                ...
                $title = $newsPreview->filterXPath('//a[contains(@class,"title-article-only")]')->text();
                $uri = UriResolver::resolve($newsPreview->filterXPath('//a[contains(@class,"title-article-only")]')->attr('href'), $this->getSiteUri());

                $publishedAtString = $newsPreview->filterXPath('//span[contains(@class,"data-text")]')->text();
                ...
                $preview = Text::trim(strip_tags(html_entity_decode($newsPreview->filterXPath('//div[contains(@class,"data-text-article")]')->text())));
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"data-news-detal")]');
        ...
        $contentCrawler = $contentCrawler->filterXPath('//div[contains(@class,"pin")]');

        $descriptionCrawler = $contentCrawler->filterXPath('//p[1]');
        if ($this->crawlerHasNodes($descriptionCrawler) && $descriptionCrawler->text() === $description) {
