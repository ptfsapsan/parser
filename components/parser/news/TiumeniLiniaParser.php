...
class TiumeniLiniaParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://t-l.ru";
    ...
    private static function inflatePost(Crawler $postData): NewsPost
    {
        ...
        $createDate = new DateTime($postData->getNode(0)->childNodes->item(9)->nodeValue);
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("div") && $node->filter("img")->count() !== 0) {
                ...
                $photoTitle = $node->filter("em");

                if ($photoTitle->count() !== 0 && !empty(trim($photoTitle->text(), "\xC2\xA0"))) {
                    self::addText($post, $photoTitle->text());
                }
            }
