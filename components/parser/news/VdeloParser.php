...
class VdeloParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://vdelo.org";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("article figure.entry-featured-media img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl($picHolder->attr("src"));
        }


        $body = $crawler->filter("article div.entry-content");
        ...
        foreach ($body->children() as $bodyNode) {
                ...
                foreach ($node->getNode(0)->childNodes as $pNode) {
                    if (!empty(trim($pNode->textContent))) {
                        if (empty($post->description)) {
                            $post->description = Helper::prepareString(trim($pNode->textContent));
                        } else {
                            self::addText($post, trim($pNode->textContent));
                        }
                    }
                }
            ...
            if ($node->matches("div, figure") && $node->filter("img")->count() !== 0) {
