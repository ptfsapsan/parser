...
class MaliiBiznessParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://mbgazeta.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("article div.thumbnail img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl($picHolder->attr("src"));
        }

        $body = $crawler->filter("article div.story");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if (mb_stripos($node->text(), "Читайте также:") !== false) {
                break;
            }
