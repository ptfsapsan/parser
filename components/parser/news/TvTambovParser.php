...
class TvTambovParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $descriptionCrawler = $newsPostCrawler->filterXPath('//h4');
        if ($this->crawlerHasNodes($descriptionCrawler) && $descriptionCrawler->text() !== '') {
            $previewNewsItem->setDescription($descriptionCrawler->text());
        }


        $contentCrawler = $newsPostCrawler->filterXPath('//div[@id="the-sticky-video"]| //div[contains(@class,"entry-content")]');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"stream-item")]/preceding-sibling::*');
