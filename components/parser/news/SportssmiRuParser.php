...
class SportssmiRuParser implements ParserInterface
{
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount && (int)date('Y') - $currentYear <= 10) {
            $currentPage = 1;

            while (count($previewList) < $maxNewsCount) {
                $uriPreviewPage = UriResolver::resolve("/{$currentYear}/page/{$currentPage}", $this->getSiteUri());
                try {
                ...
                $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[contains(@class,"article-container")]/article');
                ...
                $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $maxNewsCount) {
                    ...
                    $uri = $newsPreview->filterXPath('//*[contains(@class,"entry-title")]/a[1]')->attr('href');
                    $title = $newsPreview->filterXPath('//*[contains(@class,"entry-title")]/a[1]')->text();

                    $publishedAtString = $newsPreview->filterXPath('//time[contains(@class,"published")]')->attr('datetime');
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $mainImageCrawler = $newsPageCrawler->filterXPath('//div[contains(@id,"content")]//div[contains(@class,"featured-image")]//img[1]');
