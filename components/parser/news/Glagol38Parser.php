...
class Glagol38Parser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://glagol38.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve("/", $this->getSiteUri());
        ...
        $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[contains(@class,"news-teaser__content")]');

        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
            $titleCrawler = $newsPreview->filterXPath('//h2[contains(@class,"news-teaser__title")]//a[contains(@class,"news-teaser__link")]');

            if(!$this->crawlerHasNodes($titleCrawler)){
                return;
            }

            $uri = UriResolver::resolve($titleCrawler->attr('href'), $this->getSiteUri());
            $title = trim($titleCrawler->text());

            preg_match('/\/text\/(\d{2}-\d{2}-\d{4})\//iu',$uri,$matches);
            $publishedAtString = $matches[1];
            $publishedAt = DateTimeImmutable::createFromFormat('d-m-Y', $publishedAtString);
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"text__content")]/parent::div');
        ...
        $contentCrawler = $newsPostCrawler->filterXPath('//div[contains(@class,"text__content")]');

        $this->removeDomNodes($contentCrawler, '//a[contains(text(),"Подписывайтесь на наш Telegram-канал")]/following-sibling::*');
        $this->removeDomNodes($contentCrawler, '//a[contains(text(),"Подписывайтесь на наш Telegram-канал")]');

        $description = $description ?: $description = mb_substr($contentCrawler->text(), 0, 200) . '...';
