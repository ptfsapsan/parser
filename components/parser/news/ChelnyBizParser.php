...
class ChelnyBizParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    private const LINK = 'https://chelny-biz.ru/news/';
    private const DOMAIN = 'https://chelny-biz.ru';
    ...
    public static function run(): array
    {
        ...
        $items = $parser->find('.pin-item .pin-content.center-item');
        if (count($items)) {
            $n = 0;
            foreach ($items as $item) {
                ...
                $description = $item->find('a.pin-text')->text();
                $description = str_replace('...', '', $description);
                $createDate = $item->find('.pin-date')->text();
                $createDate = str_replace(',', '', $createDate);
                $originalParser = self::getParser($original, $curl);
                $image = $originalParser->find('.pin-content.center-item .ta-c img')->attr('src');
    ...
    {
        $paragraphs = $parser->find('.pin-text p');
