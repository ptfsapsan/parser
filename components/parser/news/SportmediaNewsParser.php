...
class SportmediaNewsParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://sportmedia.news';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $maxNewsCount) {
            ...
            $preview = Text::trim(html_entity_decode(strip_tags($newsPreview->filterXPath('//description')->text())));
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $mainImageCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"td-post-featured-image")]//img[1]')->first();
        ...
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"yasr-auto-insert-visitor")]');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"evc-social-likes")]');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"wpulike")]');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"td-a-rec-id-content_bottom")]');
