...
class SkandalyRuParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," entryinbody ")]');

        $image = null;
        $imageNode = $newsPageCrawler->filterXPath('//img[contains(concat(" ",normalize-space(@class)," ")," alignleft ")]');
        try {
            if ($this->crawlerHasNodes($imageNode)) {
                $src = $imageNode->attr('srcset');
                preg_match_all('/(?!.+\d+x\d)(http.+?\s)/m', $src, $matches);
                $image = isset($matches[0][0]) ? $matches[0][0] : null;
            }
        } catch (\Throwable $th) {
            if ($this->crawlerHasNodes($imageNode)) {
        ...
        | //img[contains(concat(" ",normalize-space(@class)," ")," alignleft ")]
        | //*[contains(concat(" ",normalize-space(@class)," ")," evc-social-likes ")]
        | //*[contains(concat(" ",normalize-space(@class)," ")," addtoany_share_save_container ")]
        | //*[contains(concat(" ",normalize-space(@class)," ")," extra_attr ")]');
