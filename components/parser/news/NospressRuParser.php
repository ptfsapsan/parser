...
class NospressRuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://nospress.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $maxNewsCount) {
            ...
            $preview = Text::trim(strip_tags(html_entity_decode($newsPreview->filterXPath('//yandex:full-text')->text())));
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPage = $this->curl->get($uri);
        $this->checkResponseCode($this->curl);

        $newsPage = $this->toUtf8($newsPage);
        $newsPageCrawler = new Crawler($newsPage);

        $contentCrawler = $newsPageCrawler->filter('#privat_page');

        $mainImageCrawler = $contentCrawler->filterXPath('//div[contains(@class,"privat_img")]/img');
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($contentCrawler, '//div[contains(@class,"privat_img")]/img');
        }
        ...
        $descriptionCrawler = $contentCrawler->filterXPath('//h2[1]');
        if ($this->crawlerHasNodes($descriptionCrawler)) {
            $description = $this->trimNormalize($descriptionCrawler->text());
        }

        $this->removeDomNodes($contentCrawler, '//*[contains(@class,"noprint")] | //*[contains(@class,"rtb-ad")] | //p[contains(@class,"author")] | //p[contains(@class,"tags")]');
        $contentCrawler = $contentCrawler->filterXPath('//h2[1]/following-sibling::p');
