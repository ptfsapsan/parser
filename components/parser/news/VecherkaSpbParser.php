...
class VecherkaSpbParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://vecherka.spb.ru/';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve("/?feed=rss2", $this->getSiteUri());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//article[contains(@class,"status-publish")]');
        ...
        $this->removeDomNodes($contentCrawler, '//div[contains(@class, "post-socials")]');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class, "post-stary_piter")]');
        $this->removeDomNodes($contentCrawler, '//header[contains(@class, "entry-header")]');
