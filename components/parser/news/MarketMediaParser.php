...
class MarketMediaParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://marketmedia.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/media-content/?PAGEN_1={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $itemXPath = '//div[contains(@class, "news-list")]';
            ...
            $previewNewsCrawler->each(
                function (Crawler $newsPreview) use (&$previewList, $uriPreviewPage) {
                    ...
                    $titleCrawler = $newsPreview->filterXPath('//p[contains(@class, "news-item")]/a');

                    $uri = UriResolver::resolve($titleCrawler->attr('href'), $uriPreviewPage);

                    $publishedAt = $newsPreview->filterXPath('//small')->last()->text();
                    $publishedAt = explode(':', $publishedAt, 2);
                    $publishedAt = trim($publishedAt[1],' ');
                    $timeZone = new DateTimeZone('Europe/Moscow');
                    $publishedAt = DateTimeImmutable::createFromFormat('d.m.Y H:i:s',$publishedAt,$timeZone);
                    $dateTime = $publishedAt;
                    $previewList[] = new PreviewNewsDTO($uri, $dateTime, $titleCrawler->text(), null);
                }
            );
        }
