...
class ThreeNewsParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    private const LINK = 'https://3news.ru/feed/';
    private const DOMAIN = 'https://3news.ru';
    ...
    {
        $p = $parser->find('.td-post-content p');
        $text = $p->text();
        if (!empty($text)) {
            $post->addItem(
                new NewsPostItem(
