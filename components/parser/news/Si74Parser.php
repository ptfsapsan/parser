...
class Si74Parser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://si74.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
            ...
            $uri = $newsPreview->filterXPath('//feedburner:origLink')->text();
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $description = $newsPageCrawler->filterXPath('//meta[@property="og:description"]')->text();
        if(!$description){
            $description = self::EMPTY_DESCRIPTION_KEY;
        }

        $contentXPath = '//div[contains(@class,"entry")]';
        $contentCrawler = $newsPostCrawler->filterXPath($contentXPath);

        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"heateorSssClear")]/following-sibling::*');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"heateorSssClear")]');
        ...
        $newsPageCrawler
            ->filterXPath('//div[contains(@class,"gallery")]//a[contains(@class,"swipebox")]')
            ->each(function (Crawler $crawler) use ($uri, $newsPost) {
                $imageLink = UriResolver::resolve($crawler->attr('href'), $uri);
                $imageLink = $this->encodeUri($imageLink);
                if ($imageLink === $newsPost->image) {
                    return;
                }
                $newsPost->addItem(new NewsPostItem(NewsPostItem::TYPE_IMAGE, null, $imageLink));
            });
