...
class InramenskoeRuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'http://inramenskoe.ru';

    private $ignoreTextPattern = '/window.*[;]/m';
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $this->removeDomNodes($newsPageCrawler, '//*[@class="b-page__title"]
        | //*[@class="b-page__single-date"]
        | //*[@class="b-page__single-date views_count"]
        | //*[@class="print"]
        | //*[@id="unit_96726"]//ancestor::div[1]
        | //*[@class="b-page__share no-print"]');

        
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@class="b-page__main"]');
        
        $this->removeDomNodes($newsPageCrawler, '//a[starts-with(@href, "javascript")]');
        $this->removeDomNodes($newsPageCrawler, '//script | //video | //style | //form');
        $this->removeDomNodes($newsPageCrawler, '//table');
        
        // $descriptionSelector = '//*[@class="b-page__content"]//p[string-length(text()) >10]';
        // $description = $newsPostCrawler->filterXPath($descriptionSelector)->text();
        // $this->removeDomNodes($newsPostCrawler, $descriptionSelector);
        $description = 'temp';
        ...
        foreach ($contentCrawler as $item) {
            ...
            foreach ($nodeIterator->getRecursiveIterator() as $k => $node) {
                ...
                if ($newsPostItem->type === NewsPostItem::TYPE_TEXT && $newsPost->description === 'temp') {
    ...
    {
        if ($node->nodeName === '#comment' || !$this->hasText($node) || preg_match($this->ignoreTextPattern, $node->textContent)) {
            return null;
        }
