...
/**
 * News parser from site https://samtime.online/
 * @author jcshow
 */
class SamTimeOnlineParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://samtime.online';
    ...
    public static function getNewsData(int $limit = 100): array
    {
        ...
        while (self::$parsedCount < $limit) {
            /** Get RSS news list */
            $curl = Helper::getCurl();
            $newsList = $curl->get(static::SITE_URL . "/category/main/page/$pageNum/");
            ...
            $news = $newsListCrawler->filterXPath('//main[contains(@id,"main")]//article');
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        $titleBlock = $itemCrawler->filterXPath('//h3[contains(@class,"entry-title")]//a[1]')->getNode(0);
        ...
        /** Get item datetime */
        $dateTimeBlock = $itemCrawler->filterXPath('//span[contains(@class,"posted-date")]//time')->getNode(0);
        $createdAt = new DateTime($dateTimeBlock->getAttribute('datetime'));
        $createdAt->setTimezone(new DateTimeZone('UTC'));
        $createdAt = $createdAt->format('c');

        /** Get description */
        $description = self::cleanText($itemCrawler->filterXPath('//div[contains(@class,"entry-content")]')->text());

        //Get image if exists
        $picture = null;
        $imageBlock = $itemCrawler->filterXPath('//div[contains(@class,"entry-img")]//img[1]')->getNode(0);
        ...
        self::removeNodes($crawler, '
            ...
            //div[contains(@class,"sharedaddy sd-sharing-enabled")]
            /following-sibling::*
        ');
        self::removeNodes($crawler, '//div[contains(@class,"entry-content")]//div[contains(@class,"sharedaddy sd-sharing-enabled")]');
