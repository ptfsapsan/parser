...
class AdigeaTodayParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://adigeatoday.ru/';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            ...
            $previewNewsCrawler = $previewNewsCrawler->filterXPath(
                '//div[@id="article-container"]//div[contains(@class,"post-sm-container page-container col-mobile-100")]'
            );

            $previewNewsCrawler->each(
                function (Crawler $newsPreview) use (&$previewList, $url) {
                    $titleCrawler = $newsPreview->filterXPath('//p/strong/a');
                    $uri = UriResolver::resolve($titleCrawler->attr('href'), $this->getSiteUri() . $url);
                    $title = trim($titleCrawler->text());


                    $publishedAtString = $newsPreview->filterXPath('//p[@class="sm-date"]')->text();
                    $publishedAt = DateTimeImmutable::createFromFormat('d.m.Y H:i', $publishedAtString);
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $this->removeDomNodes(
            $contentCrawler,
            '//div[contains(@class,"col-desktop-45-right col-landscape-45-right col-inline")]'
        );
        $this->removeDomNodes($contentCrawler, '//span[contains(@class,"image-info")]');
        $this->removeDomNodes($contentCrawler, '//span[contains(@itemprop,"datePublished")]');
        $this->removeDomNodes($contentCrawler, '//img[1]');
        $this->removeDomNodes($contentCrawler, '//h1[contains(@itemprop,"headline")]');
        $this->removeDomNodes($contentCrawler, '//img[last()]');
