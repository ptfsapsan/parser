...
/**
 * Парсер новостей из RSS ленты tv29.ru
 *
 */
class Tv29 extends TyRunBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const MAIN_PAGE_URI = 'http://tv29.ru';

    /**
     * CSS класс, где хранится содержимое новости
     */
    const BODY_CONTAINER_CSS_SELECTOR = '.art-content';
     ...
     * в формате RegExp
     */
    const EXCLUDE_CSS_CLASSES_PATTERN = '/storyline-vrez/';

    /**
     * Класс элемента после которого парсить страницу не имеет смысла (контент статьи закончился)
     */
    const CUT_CSS_CLASS = 'jc';


    /**
     * Ссылка на RSS фид (XML)
     */
    const FEED_URL = 'http://tv29.ru/new/index.php?format=feed&type=rss';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('rss channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function (Crawler $node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $articleContent = $newsContent->filter('.item-page')->children();
                $stopParsing = false;
    ...
    protected static function parseNode(Crawler $node, NewsPost $newPost, int $maxDepth, bool &$stopParsing, $descriptionSentences = []): void
    {
        ...
        switch ($node->nodeName()) {
            ...
            case 'em':
                    ...
                    if ($linkImage->count()) {
                        self::parseImage($linkImage, $newPost);
                    }
