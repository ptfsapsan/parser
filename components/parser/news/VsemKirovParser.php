...
/**
 * News parser from site http://vsem-kirov.ru/
 * @author jcshow
 */
class VsemKirovParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'http://vsem-kirov.ru';
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        self::removeNodes($crawler, '//a[contains(@style,"none")]');

        //Get image if exists
        $picture = null;
        $imageBlock = $crawler->filterXPath('//div[contains(@class,"newspadd")]//div[1]//img')->getNode(0);
        ...
        /** Get description */
        $descriptionBlock = $crawler->filterXPath('//div[contains(@class,"newspadd")]//div[1]')->getNode(0);
        if (! empty($descriptionBlock) === true) {
            $description = self::cleanText($descriptionBlock->textContent);
            $descriptionBlock->parentNode->removeChild($descriptionBlock);
        } else {
            $description = self::cleanText($itemCrawler->filterXPath('//category')->text());
        }

        /** @var NewsPost */
        $post = new NewsPost(static::class, $title, $description, $createdAt, $link, $picture);

        $detailPage = $crawler->filterXpath('//div[contains(@class,"newspadd")]//div')->getNode(0);

        // parse detail page for texts if needed
        if (! empty($detailPage) === true) {
