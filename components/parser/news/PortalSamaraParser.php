...
/**
 * News parser from site https://portalsamara.ru/
 * @author jcshow
 */
class PortalSamaraParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://portalsamara.ru';
    ...
    public static function getPostDetail(DOMElement $item): NewsPost
    {
        ...
        $contentHtml = $itemCrawler->filterXPath('//turbo:content')->text();
        $curlResult = preg_replace('/\<br\>/m', '', $contentHtml);
        $curlResult = preg_replace('/' . preg_quote($description, '/') . '/m', '', $contentHtml);
        ...
        foreach ($crawler->filterXPath('//*')->getNode(0)->childNodes as $node) {
