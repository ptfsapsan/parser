...
class GroznyParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://grozny.tv';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/news.php?page={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[@class="bl-news-all"]');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $uriCrawler = $newsPreview->filterXPath('//a[contains(text(),"подробнее")]');
                $uri = UriResolver::resolve($uriCrawler->attr('href'), "{$this->getSiteUri()}/news.php");

                $timezone = new DateTimeZone('Europe/Moscow');
                $publishedAtString = $newsPreview->filterXPath('//time')->text();
                $publishedAt = DateTimeImmutable::createFromFormat('Y m d \в H:i', $publishedAtString, $timezone);
                ...
                $previewList[] = new PreviewNewsDTO($uri, $publishedAtUTC, null, $preview);
            });
        }
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"js-mediator-article")]');
        ...
        $title = $newsPostCrawler->filterXPath('//h1[contains(@class,"news_title_h1")]')->text();

        $this->removeDomNodes($contentCrawler, '//*[contains(@class,"news_title_h1") or contains(@class,"meta-info")]');

        $description = mb_substr($contentCrawler->text(), 0, 200) . '...';
