 ...
 *
 */
class AtoRu extends Aleks007smolBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const MAIN_PAGE_URI = 'http://www.ato.ru';

    /**
     * CSS класс, где хранится содержимое новости
     */
    const BODY_CONTAINER_CSS_SELECTOR = '.field-name-body .field-item';
     ...
     */
    const FEED_URL = 'http://www.ato.ru/feed';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function ($node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                ...
                $newsContent = $type ? (new Crawler($newsContent))->filter(self::BODY_CONTAINER_CSS_SELECTOR) :
                    (new Crawler($newsContent))->filter('.content .field-item');
