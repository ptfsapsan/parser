...
class GazetaParmaParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://xn--80aaaalnn7bzals.xn--p1ai';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/archives/category/новости/page/{$pageNumber}",
                $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                ...
                $preview = null;
                if ($this->crawlerHasNodes($newsPreview->filterXPath('//div[contains(@class,"entry-content")]/p'))) {
                    $preview = $newsPreview->filterXPath('//div[contains(@class,"entry-content")]/p')->text();
                }
