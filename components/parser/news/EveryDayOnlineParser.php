...
class EveryDayOnlineParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://www.everydayonline.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $image = $crawler->filter("div#top-news div.news-additional-info-block img");

        if ($image->count() !== 0) {
            $post->image = self::PREFIX . self::normalizeUrl($image->attr("src"));
        }

        $body = $crawler->filter("div#top-news div.news-text");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("p") && mb_stristr($node->text(), "- Everyday Online.") !== false) {
                continue;
            }
