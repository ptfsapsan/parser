...
class GtrkPskovParser implements ParserInterface
{
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve('/?format=feed&type=rss', $this->getSiteUri());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"item-page")]');

        $mainImageCrawler = $contentCrawler->filterXPath('//img[contains(@class,"ja-video")]')->first();
        ...
        $contentCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"item-page")]//*[contains(@class,"article-content")]');
