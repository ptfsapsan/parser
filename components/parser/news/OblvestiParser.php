...
/**
 * News parser from site https://oblvesti.ru/
 * @author jcshow
 */
class OblvestiParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://oblvesti.ru';
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        //Get image if exists
        $picture = null;
        $imageBlock = $crawler->filterXPath('//figure[contains(@class, "single-featured-image")]//img')->getNode(0);
        ...
        /** Get description */
        $descriptionBlock = $itemCrawler->filterXPath('//description')->text();
        $descCrawler = new Crawler($descriptionBlock);
        $descriptionNode = $descCrawler->filterXPath('//p[1]')->getNode(0);
        $description = self::cleanText($descriptionNode->textContent);

        /** @var NewsPost */
        $post = new NewsPost(static::class, $title, $description, $createdAt, $link, $picture);

        /** Encoded content crawler */
        $encodedContent = $itemCrawler->filterXPath('//content:encoded')->text();
        $crawler = new Crawler($encodedContent);
        
        self::removeNodes($crawler, '//p[last()]');
        self::removeNodes($crawler, '//comment()');
        self::removeNodes($crawler, '//br');
        self::removeNodes($crawler, '//hr');
