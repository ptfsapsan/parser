...
class VsluhParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://vsluh.ru/';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("chrono?page={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $itemXPath = '//body/section/div//div[contains(@class, "container m-0 p-0")]';
            ...
            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $uriPreviewPage) {
                $titleCrawler = $newsPreview->filterXPath('//div[contains(@class, "chrono-title")]/node()[not(self::span)]');
                $uri = $newsPreview->filterXPath('//a[contains(@class,"nocolor")]')->attr('href');
                $uriResolver = UriResolver::resolve($uri, $uriPreviewPage);
                $preview = $newsPreview->filterXPath('//div[contains(@class, "chrono-lead")]')->text();
                $previewList[] = new PreviewNewsDTO($uriResolver, null, $titleCrawler->text(), $preview);
            });
        }
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//article[@class="post-text mb-4"]');
        ...
        $publishedAtString = trim($newsPageCrawler->filterXPath('//time')->attr('datetime'));
        $publishedAtString = preg_split('/[+,T,.]/',$publishedAtString);
        unset($publishedAtString[3],$publishedAtString[2]); //убираем лишние элементы у даты
        $publishedAtString = implode(' ',$publishedAtString);
        ...
        $newsPost = new NewsPost(self::class, $title, $description, $publishedAtUTC->format('Y-m-d H:i:s'), $uri,
                                 $image);
        ...
        $this->removeDomNodes($contentCrawler, '//*[contains(text(),"Еще по теме")]/following-sibling::* | //*[contains(text(),"Еще по теме")]');
