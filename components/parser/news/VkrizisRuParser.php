...
class VkrizisRuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://vkrizis.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $maxNewsCount) {
            ...
            $preview = Text::trim(strip_tags(html_entity_decode($newsPreview->filterXPath('//description')->text())));
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filterXPath('//main[@id="main"]');

        $mainImageCrawler = $contentCrawler->filterXPath('//header[contains(@class,"entry-header")]//div[contains(@id,"mainpic")]/img[1]');
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($contentCrawler, '//header[contains(@class,"entry-header")]//div[contains(@id,"mainpic")]/img[1]');
        }
        ...
        $contentCrawler = $contentCrawler->filterXPath('//article[contains(@id,"post-")]/div[contains(@class,"entry-content")]');
