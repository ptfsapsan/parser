...
class KlitnsyTrudRuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://klitnsy-trud.ru';
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," contentholder ")]');

        $imageNode = $newsPostCrawler->filterXPath('//a[child::img][1]');
        if ($this->crawlerHasNodes($imageNode)) {
            $src = $imageNode->attr('href');
        ...
        | //h1
        | //a[child::img]
        | //p[last()]/following-sibling::*
        | //div[@class="readtoo"]
        | //div[@class="related"]
        | //div[contains(concat(" ",normalize-space(@class)," ")," datetimesmall ")]');
