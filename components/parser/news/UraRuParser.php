...
class UraRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://ura.news";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div.publication div[itemprop=\"articleBody\"]");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("p") && $node->filter("div.vc-incut-story")->count() !== 0) {
                continue;
            }
            if ($node->matches("p.yandex-rss-hidden")) {
                continue;
            }
            ...
            if ($node->matches("div") && $node->filter("img")->count() !== 0) {
                ...
                $description = $node->filter("div.item-img-description");
                if ($description->count() !== 0) {
                    self::addText($post, $description->text());
                }
            }
