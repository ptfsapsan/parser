...
class PerevolokDayParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://perevolockday.ru";

    const FEED_SRC = "/main-news";
    const LIMIT = 100;
    ...
    public static function run(): array
    {
        ...
        for ($pageId = 1; $pageId <= self::LIMIT; $pageId += self::NEWS_PER_PAGE) {
            ...
            $listSourcePath = self::ROOT_SRC . self::FEED_SRC . "?page_start=" . $pageId;
            ...
            $items = $crawler->filter("div.nb-type1");
            if ($items->count() === 0) {
    ...
    {
        $title = $postData->filter("div.txt-block h3 a")->text();

        $original = self::normalizeUrl(self::ROOT_SRC . "/" . $postData->filter("div.txt-block h3 a")->attr("href"));

        $dateString = $postData->filter("div.meta-line span.date")->text();
        $timeString = $postData->filter("div.meta-line span.time")->text();
        ...
        $dateString = date("Y") . "-" . self::MONTHS[$dateArr[1]] . "-" . $dateArr[0] . " " . $timeString . "+05:00";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div.st-page div.content");
