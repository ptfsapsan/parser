...
class HakikatInfoParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://hakikat.info';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/kat/lenta-novostej?start={$pageNumber}", $this->getSiteUri());
            $pageNumber += 9;
            ...
            $previewNewsCrawler = $previewNewsCrawler->filter('.itemList .itemContainer');

            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $title = $this->trimNormalize($newsPreview->filter('.catItemTitle a')->text());
                $uri = UriResolver::resolve($newsPreview->filter('.catItemTitle a')->attr('href'), $this->getSiteUri());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $mainImageCrawler = $newsPageCrawler->filter('#k2article-slider img')->first();
        ...
        $contentCrawler = $newsPageCrawler->filter('.itemIntroText');
    ...
    private function getPublishedAt(Crawler $crawler): ?DateTimeImmutable
    {
        ...
        $publishedAt = $this->trimNormalize($crawler->filter('.article-info .create')->text());
