...
/**
 * Парсер новостей из RSS ленты www.gubernia.com
 *
 */
class Gubernia extends TyRunBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const MAIN_PAGE_URI = 'http://www.gubernia.com';
     ...
     */
    const FEED_URL = 'http://www.gubernia.com/rss_mainnews.php';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('rss channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function (Crawler $node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $articleContent = $newsContent->filter('.txt-detail')->children();
                $stopParsing = false;
