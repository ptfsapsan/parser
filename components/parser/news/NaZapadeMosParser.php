...
class NaZapadeMosParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://na-zapade-mos.ru/';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve("/share/rss.xml", $this->getSiteUri());
        ...
        $previewNewsCrawler->each(
            function (Crawler $newsPreview) use (&$previewList) {
                ...
                $preview = trim(strip_tags($newsPreview->filterXPath('//description')->text()));
                $previewExplode = explode('.',$preview,2);
                $preview = $previewExplode[0];
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"main-content-area")]');


        $mainImageCrawler = $newsPageCrawler->filterXPath('//p[contains(@class,"fullmimg")]/img')->first();
        ...
        $contentCrawler = $newsPostCrawler->filterXPath('//div[contains(@class,"ce_text block")]/p');

        $this->removeDomNodes($newsPageCrawler, '//p[contains(@class, "avtory")]');
