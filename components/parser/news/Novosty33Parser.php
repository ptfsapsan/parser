...
class Novosty33Parser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://novosti33.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div.main div.news-single-content div.clearfix");
        ...
        $morePics = $crawler->filter("div.main div.news-single-content div.more-images img");
        if ($morePics->count() !== 0) {
            $morePics->each(function (Crawler $picNode) use ($post) {
