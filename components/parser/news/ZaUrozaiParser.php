...
/**
 * Парсер новостей с сайта https://moyaokruga.ru/za-urozai/
 */
class ZaUrozaiParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://moyaokruga.ru/za-urozai';
    ...
    public function parse(): array
    {
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//section[@class="articles-main"]');
            $title = $content->filterXPath("//header/a")->text();
            $date = $this->getDate($content->filterXPath("//header/time")->text());
            $image = null;
            $imgSrc = $content->filterXPath("//div[@class='main-im']/*/img");
            if ($imgSrc->getNode(0)) {
                $image = $this->getHeadUrl($imgSrc->attr('src'));
            }

            $description = $content->filterXPath("//article")->children();
            ...
            $newContentCrawler = $itemCrawler->filterXPath('//div[@id="MainMasterContentPlaceHolder_InsidePlaceHolder_articleText"]');
            foreach ($newContentCrawler as $contentNew) {
                foreach ($contentNew->childNodes as $childNode) {
                    ...
                    } elseif ($childNode->nodeName == 'a' && strpos($this->getHeadUrl($childNode->getAttribute('href')), 'http') !== false) {
            ...
            $videoCrawler = $itemCrawler->filterXPath('//div[@id="MainMasterContentPlaceHolder_InsidePlaceHolder_divVideo"]');
            if ($videoCrawler->getNode(0) && $src = $videoCrawler->attr('src')) {
                if (strpos($src, 'youtube') !== false) {
                    $youId = basename(parse_url($src, PHP_URL_PATH));
                    $titleYou = $videoCrawler->attr('title');
                    $this->addItemPost($post, NewsPostItem::TYPE_VIDEO, $titleYou, null, null, null, $youId);
                }
            }

            $authorCrawler = $itemCrawler->filterXPath('//a[@id="MainMasterContentPlaceHolder_InsidePlaceHolder_authorName"]');
            if ($authorCrawler->getNode(0)) {
                $this->addItemPost($post, NewsPostItem::TYPE_TEXT, 'Автор:');
                $this->addItemPost($post, NewsPostItem::TYPE_LINK, $authorCrawler->text(), null, $this->getHeadUrl($authorCrawler->attr('href'), '/'));
            }
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath("//div[@class='txt-articles']")->filterXPath("//h3/a");
