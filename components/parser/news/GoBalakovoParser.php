...
class GoBalakovoParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://www.gobalakovo.ru";
    ...
    public static function run(): array
    {
        ...
        $items = $crawler->filter("div.c-news-block");
    ...
    {
        $title = $postData->filter("a")->text();

        $original = $postData->filter("a")->attr("href");
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $dateHolder = $crawler->filter("main div.container-fluid meta[itemprop='datePublished']");
        if($dateHolder->count() === 0){
            throw new Exception("Не найден блок даты публикации");
        }

        $createDate = new DateTime($dateHolder->attr("content"));
        $createDate->setTimezone(new DateTimeZone("UTC"));
        $post->createDate = $createDate;

        $picHolder = $crawler->filter("main div.container-fluid div.sect-mb div.article-photo img");
        if($picHolder->count() !== 0){
            $post->image = self::normalizeUrl($picHolder->attr("data-src"));
        }


        $body = $crawler->filter("main div.container-fluid div.article-text")->parents();
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("div.fotorama") && $node->filter("img")->count() !== 0) {
                $node->filter("img")->each(function (Crawler $picNode) use ($post){
                    $src = self::normalizeUrl($picNode->attr("src"));
                    if($post->image === null){
                        $post->image = $src;
                    }else{
