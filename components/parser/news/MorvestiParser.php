...
class MorvestiParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'http://morvesti.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("/news/1679/?PAGEN_1={$pageNumber}", $this->getSiteUri());
            $pageNumber++;
            ...
            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                $titleCrawler = $newsPreview->filterXPath('//a[contains(@href,"/news/")]');
                $uri = UriResolver::resolve($titleCrawler->attr('href'), "{$this->getSiteUri()}/news/1679/");
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"main-block")]/div[@class="col-md-10"]');

        $mainImageCrawler = $newsPageCrawler->filterXPath('//img[contains(@class,"detail_picture")]')->first();
        ...
        $publishedAtString = $newsPostCrawler->filterXPath('//p[contains(@class,"today")]')->text();
        $publishedAtUTC = DateTimeImmutable::createFromFormat('d.m.Y', $publishedAtString, new DateTimeZone('UTC'));

        $contentCrawler = $newsPostCrawler->filterXPath('//div[contains(@class,"main-title-text")]');
