...
class FinControlParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://fcinfo.ru/";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $image = $crawler->filter("div.full-news-text img");
        if ($image->count() !== 0) {
            $post->image = self::normalizeUrl($image->attr("src"));
        }

        $body = $crawler->filter("div.full-news-text p");
