...
class BiznesmanParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://24biznesman.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("article#the-post div.entry-content");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->text() === "Источник") {
                break;
            }

            if ($node->matches("p") && $node->filter("img")->count() !== 0) {
                $image = $node->filter("img");
                $src = self::normalizeUrl($image->attr("data-src"));
                if ($post->image === null) {
                    $post->image = $src;
                } else {
