...
class ExpertnwComParser extends AbstractBaseParser
{
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('.section-main ');

        $mainImageCrawler = $contentCrawler->filterXPath('//div[contains(@class,"article-main-img")]/img[1]')->first();
        $image = null;
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($contentCrawler, '//div[contains(@class,"article-main-img")]');
        }
        if ($image !== null && $image !== '') {
        ...
        $contentCrawler = $contentCrawler->filter('.section-main-box .plain-text');
        $this->removeDomNodes($contentCrawler, '//div[contains(@class,"plain-text-banner")]');
