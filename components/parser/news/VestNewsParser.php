...
class VestNewsParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://www.vest-news.ru";
    ...
    public static function run(): array
    {
        ...
        $items = $crawler->filter("div.short_new2");
        if ($items->count() === 0) {
    ...
    {
        $title = $postData->filter("a")->eq(1)->text();

        $original = self::ROOT_SRC . $postData->filter("a")->eq(1)->attr("href");

        $dateStr = $postData->filter("div.date")->text();
        $createDate = DateTime::createFromFormat("d.m.Y H:i P", $dateStr . " +03:00");
        if ($createDate === false) {
            throw new Exception("Не найдена дата публикации: " . $original);
        }
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div.article div.text");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("div.gallery_wrap") && $node->filter("img")->count() !== 0) {
