...
class Tvk6Parser extends AbstractBaseParser
{
    ...
    {
        return 'https://tvk6.ru';
    }

    protected function getPreviewNewsDTOList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        $previewList = [];
        $url = "/publications/rss/";
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $image = null;
        $mainImageCrawler = $newsPageCrawler->filterXPath('//meta[@name="relap-image"]');
        ...
        $descriptionCrawler = $newsPostCrawler->filterXPath('//div[contains(@class,"news_lid")]');
        if ($this->crawlerHasNodes($descriptionCrawler) && $descriptionCrawler->text() !== '') {
            $previewNewsItem->setDescription($descriptionCrawler->text());
        }

        $contentCrawler = $newsPostCrawler->filterXPath('//div[contains(@class,"tvk_news")]');
        $this->removeDomNodes($contentCrawler,'//a[contains(@href,"https://tvk6.ru/donate")]');
