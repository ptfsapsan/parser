...
class AstralistInfoParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://astralist.info';
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@class="featured-image"]//img | //div[contains(concat(" ",normalize-space(@class)," ")," entry-content ")]');
        

        $this->removeDomNodes($newsPostCrawler, '//*[contains(text(), "Фото:")]
        | //*[contains(concat(" ",normalize-space(@class)," ")," it-theme1 ")]/ancestor::*[1]');
            ...
            foreach ($nodeIterator->getRecursiveIterator() as $k => $node) {
                ...
                if ($newsPostItem->type === NewsPostItem::TYPE_TEXT) {
                    ...
                    if ($isTextContainsDescription) {
                        $replacedText = str_replace($description, '', $newsPostItem->text);
