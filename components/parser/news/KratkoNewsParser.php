...
class KratkoNewsParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://kratko-news.com";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("section.post div.entry-thumb img");
        if($picHolder->count() !== 0){
            $post->image = self::normalizeUrl($picHolder->attr("src"));
        }

        $body = $crawler->filter("section.post div.entry-content");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("ul.wp-block-gallery") && $node->filter("img")->count() !== 0) {
            ...
            if ($node->matches("figure") && $node->filter("iframe")->count() !== 0) {
