...
/**
 * Парсер новостей с сайта https://vesiskitim.ru/
 */
class VesIskitimRuParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    const SITE_URL = 'https://vesiskitim.ru';
    ...
    public function parse(): array
    {
        ...
        $urlNews = self::SITE_URL . '/vse_novosti';
        ...
        foreach ($newsList as $newsUrl) {
            ...
            $content = $itemCrawler->filterXPath('//div[@class="article"]');
            $title = $content->filterXPath("//h1[@class='article__title']")->text();
            $date = '';
            $dateSrc = $content->filterXPath("//span[@class='article-info__block']");
            foreach ($dateSrc as $item) {
                $date = $date . ' '. $this->clearText($item->nodeValue);
            }
            ...
            $newContentCrawler = $content->filterXPath("//div[@class='content article-body content-article-body']")->children();
    ...
    protected function getListNews(string $page): array
    {
        ...
        $list = $crawler->filterXPath("//div[@class='news-t4']")->filterXPath("//a");
