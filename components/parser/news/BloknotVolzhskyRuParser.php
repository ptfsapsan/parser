 ...
 * @rss_html
 */
class BloknotVolzhskyRuParser extends MediasferaNewsParser implements ParserInterface
{
    ...
    public const SITE_URL = 'https://bloknot-volzhsky.ru/';
    public const NEWSLIST_URL = 'https://bloknot-volzhsky.ru/rss_news.php';
        ...
        $listCrawler->filterXPath(self::NEWSLIST_POST)->slice(0, self::NEWS_LIMIT)->each(function (Crawler $node) use (&$posts) {
            ...
            foreach ($newsPost->items as $key => $item) {
                ...
                if($item->type == NewsPostItem::TYPE_TEXT) {
                    ...
                    else if(strpos($text, 'Вас обманули, оскорбили, унизили?') !== false) {
                        unset($newsPost->items[$key]);
                        $removeNext = true;
                    }
