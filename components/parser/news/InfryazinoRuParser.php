...
class InfryazinoRuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'http://infryazino.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            ...
            $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList) {
                ...
                $uri = UriResolver::resolve($newsPreview->filterXPath('//*[contains(@class,"news-itm__title")]/a')->attr('href'), $this->getSiteUri());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $mainImageCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"b-page__main")]//div[contains(@class,"b-page__image")]//img[1]')->first();
        ...
        $description = Text::trim($newsPageCrawler->filter('meta[name=description]')->attr('content'));
        if (!$description) {
            $description = Text::trim($contentCrawler->filterXPath('//div[contains(@class,"b-page__start")]')->text());
        }
