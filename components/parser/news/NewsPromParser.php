...
class NewsPromParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'http://newsprom.ru/';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        while (count($previewList) < $maxNewsCount) {
            $uriPreviewPage = UriResolver::resolve("news/lenta/p{$pageNumber}.html", $this->getSiteUri());
            $pageNumber++;
            ...
            $itemLiXPath = '//div[contains(@class,"important-list")]/li[contains(@class,"clearfix")]';
            $itemXPath = '//div[contains(@class, "left-block")]/div[contains(@class,"clearfix")][1]' . " | {$itemLiXPath}";
            ...
            $previewNewsCrawler->each(
                function (Crawler $newsPreview) use (&$previewList, $uriPreviewPage) {
                    $titleCrawler = $newsPreview->filterXPath('//div[contains(@class, "title")]/a');
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@class="block-detail-view"]');

        $mainImageCrawler = $newsPostCrawler->filterXPath('//div[@class="main-image"]/a/img')->first();
        ...
        $publishedAt = DateTimeImmutable::createFromFormat('H:i d F Y', $publishedAtString, $timezone);
        $publishedAtUTC = $publishedAt->setTimezone(new DateTimeZone('UTC'));

        $newsPost = new NewsPost(
            self::class, $title, $description, $publishedAtUTC->format('Y-m-d H:i:s'), $uri,
            $image
        );
