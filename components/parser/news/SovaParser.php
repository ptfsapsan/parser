...
class SovaParser extends MediasferaNewsParser implements ParserInterface
{
    ...
    public const NEWSLIST_TEXT = '//yandex:full-text';
    ...
    public static function run(): array
    {
        ...
        $listCrawler->filterXPath(self::NEWSLIST_POST)->slice(0, self::NEWS_LIMIT)->each(function (Crawler $node) use (&$posts) {
            ...
            static::parseItemContent(self::filterNode($node, self::NEWSLIST_TEXT));
    ...
    protected static function parseItemContent(Crawler $node)
    {
        $textParts = explode("\n", $node->html());

        foreach ($textParts as $part) {
            $text = trim($part, self::CHECK_EMPTY_CHARS);
            ...
            if($text) {
                ...
                self::$post->itemText = $text;

                if(!self::$post->description) {
                    self::$post->description = $text;
                }
            }
        }
    }
