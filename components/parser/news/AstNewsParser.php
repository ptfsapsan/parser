...
class AstNewsParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://ast-news.ru/';
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@class="post-content p-0"]');

        $mainImageCrawler = $newsPostCrawler->filterXPath('//div[@class="post-featured-image"]/a/img[1]');

        if ($this->crawlerHasNodes($mainImageCrawler)) {
            if ($mainImageCrawler->attr('src') !== '') {
                $image = $mainImageCrawler->attr('src');
                $this->removeDomNodes($newsPostCrawler, '//div[@class="post-featured-image"]/a/img[1]');
                $this->removeDomNodes($newsPostCrawler, '//div[@class="post-featured-image"]/a');
            }
        }

        $description = $newsPageCrawler->filterXPath(
            '//meta[@property="description"] | //meta[@property="og:description"]'
        );

        //проверяем есть ли description
        if ($this->crawlerHasNodes($description)) {
            $description = $description->attr('content');
            $bCount = 1;
            //если его нет начинаем перебирать элементы //b пока ненайдем
            if ($description === "") {
                while ($description === "" && $bCount < 5) {
                    $descriptionCrawler = $newsPostCrawler->filterXPath("//b[{$bCount}]");
                    //нашли записали
                    if ($this->crawlerHasNodes($descriptionCrawler) && $descriptionCrawler->text() !== "") {
                        $description = $descriptionCrawler->text();
                        $this->removeDomNodes($newsPostCrawler, "//b[{$bCount}]");
                        unset($bCount);
                        break;
                    }
                    $bCount++;
        ...
        //если description был не в теге b, а в теге p делаем проверку
        ...
        foreach ($contentCrawler as $item) {
            ...
            foreach ($nodeIterator->getRecursiveIterator() as $k => $node) {
                ...
                if ($newsPostItem->type == 1 && $newsPostItem->text == "") {
                    continue;
                }
