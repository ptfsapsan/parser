...
/**
 * News parser from site https://priazove.ru/
 * @author jcshow
 */
class PriazovjeParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://priazove.ru';
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        /** Content:encoded crawler creation */
        $crawler = new Crawler($itemCrawler->filterXPath('//content:encoded')->text());
        ...
        /** Get description */
        $descriptionBlock = $crawler->filterXpath('//p[string()]')->getNode(0);
        if (empty($descriptionBlock->textContent) === true) {
            $description = $title;
        } else {
        ...
        if ($crawler->getNode(0)->childNodes->count() > 0) {
