...
class NovosibirenParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://blog.visart.biz";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div.post div.entry");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("h3") && mb_stripos($node->text(), "Comments") !== false) {
                break;
            }
            ...
            if ($node->matches("p") && $node->filter("img")->count() !== 0) {
                ...
                $image->each(function (Crawler $inNode) use ($post) {
                    $src = self::normalizeUrl($inNode->attr("src"));
                    if ($post->image === null) {
                        $post->image = $src;
                    } else {
