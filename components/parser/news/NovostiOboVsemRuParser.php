...
class NovostiOboVsemRuParser implements ParserInterface
{
    use \app\components\helper\metallizzer\Cacheable;

    const USER_ID  = 2;
    const FEED_ID  = 2;
    const SITE_URL = 'http://novosti-obo-vsem.ru';
    ...
    protected static function loadPost($item)
    {
        ...
        self::$posts[] = (new Parser())->fill(
            $post,
            $page->filterXpath('//div[@class = "entry-inner"]/node()[not(contains(@class, "kk-star-ratings")) and not(contains(@class, "yarpp-related"))]')
        );
    }
