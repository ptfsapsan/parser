...
class GpkorkinoRuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://gpkorkino.ru';
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $mainImageCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"single-post-image")]//img[1]')->first();
        ...
        $contentCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"entry-content")]');
        $descriptionCrawler = $contentCrawler->filterXPath('//p[contains(@class,"lead")]')->first();
        if ($this->crawlerHasNodes($descriptionCrawler) && $descriptionCrawler->text()) {
            $description = $descriptionCrawler->text();
        }
        $this->removeDomNodes($contentCrawler, '//p[contains(@class,"lead")]');
        $this->removeDomNodes($contentCrawler, '//p[contains(@class,"autor_text")]');

        $contentCrawler = $newsPageCrawler->filterXPath('//*[contains(@class,"a_mr_css_attr")]');
