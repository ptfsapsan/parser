...
class MsPortalParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://microsoftportal.net";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div#dle-content div.full_text");
        ...
        foreach ($body->getNode(0)->childNodes as $bodyNode) {
            ...
            if (mb_stripos($bodyNode->textContent, "Узнать другие подробности можно") !== false) {
                break;
            }


            if ($node->matches("div.quote") && !empty(trim($node->text(), "\xC2\xA0"))) {
