...
class OrenburzhieRuParser extends AbstractBaseParser
{
    /*run*/
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filter('#main article');

        $image = null;
        $mainImageCrawler = $newsPageCrawler->filter('.wp-post-image')->first();
        ...
        $description = null;
        $contentCrawler = $newsPostCrawler->filter('.entry-content');
        $descriptionCrawler = $contentCrawler->filterXPath('//p[1]/strong');
        if ($this->crawlerHasNodes($descriptionCrawler)) {
            $description = $this->normalizeSpaces($descriptionCrawler->text());
