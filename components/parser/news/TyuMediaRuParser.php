...
class TyuMediaRuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'http://tyumedia.ru';
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," detail-text ")]');

        $fullText = implode('', $newsPostCrawler->each(function (Crawler $node, $i) {
            return $node->text();
        }));

        $imageNode = $newsPageCrawler->filterXPath('//div[contains(concat(" ",normalize-space(@class)," ")," detail-header ")]//*[@class="image"]//img');
        ...
        if (trim($description) === '' || strlen($fullText) < 4) {
            $description = $title;
        }
        ...
        foreach ($contentCrawler as $item) {
            ...
            foreach ($nodeIterator->getRecursiveIterator() as $k => $node) {
                ...
                if ($newsPostItem->type === NewsPostItem::TYPE_TEXT) {
                    ...
                    $replace = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $newsPostItem->text);

                    if ($isDescriptionContainsText || strlen($replace) < 2) {
                        continue;
                    }
