...
class SarapulLiveRuParser extends AbstractBaseParser
{
    ...
    protected function getPreviewNewsDTOList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve('/rss/news.html', $this->getSiteUrl());
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('.item-show .content-container');

        $image = null;
        $mainImageCrawler = $contentCrawler->filterXPath('//div[contains(@class,"image-container")]//img[1]');
        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($contentCrawler, '//div[contains(@class,"image-container")]');
        }
