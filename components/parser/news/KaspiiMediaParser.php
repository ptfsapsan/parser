...
class KaspiiMediaParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://kaspiy-media.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $imageHolder = $crawler->filter("div#slider ul > li > a");
        if ($imageHolder->count() !== 0) {
            $imageHolder->each(function (Crawler $aNode) use ($post) {
                if ($post->image === null) {
                    $post->image = self::normalizeUrl($aNode->attr("href"));
                } else {
                    self::addImage($post, self::normalizeUrl($aNode->attr("href")));
                }
            });
        }

        $body = $crawler->filter("div.content_single > div.single_text");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("p") && !empty(trim($node->text(), "\xC2\xA0"))) {
                ...
                foreach ($node->getNode(0)->childNodes as $innerNode) {
                    if ($innerNode->nodeName === '#text') {
                        $innerCrawl = new Crawler($innerNode);
                        if (empty($post->description)) {
                            $post->description = Helper::prepareString($innerCrawl->text());
                        } else {
                            self::addText($post, $innerCrawl->text());
                        }
                    }
                }
                continue;
