...
class RZOnlineRuParser implements ParserInterface
{
    /*run*/
    public const USER_ID = 2;
    public const FEED_ID = 2;
    public const SITE_URL = 'https://www.rznonline.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $uriPreviewPage = UriResolver::resolve('/news/rss', $this->getSiteUri());
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $newsPostCrawler = $newsPageCrawler->filterXPath('//div[@id="top-news"]');

        $imageNode = $newsPostCrawler->filterXPath('//a[child::img[@name="news-photo"]]');
        if ($this->crawlerHasNodes($imageNode)) {
            $image = UriResolver::resolve($imageNode->attr('href'), $this->getSiteUri());
        }

        $this->removeDomNodes($newsPostCrawler, '//*[contains(text(), "Фото:")]
        | //div[contains(concat(" ",normalize-space(@class)," ")," seealso ")]
        | //div[contains(concat(" ",normalize-space(@class)," ")," header ")]
        | //p[descendant::a[contains(@href, "news.yandex")]]
        | //div[contains(concat(" ",normalize-space(@class)," ")," news-additional-info-block ")]
        | //p[contains(text(), "Рязань Онлайн.")]
        | //*[@id="news-title"]');
