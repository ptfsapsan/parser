...
/**
 * Парсер новостей из RSS ленты miasskiy.ru
 *
 */
class Miasskiy extends TyRunBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const BODY_CONTAINER_CSS_SELECTOR = '.jeg_inner_content';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('rss channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function (Crawler $node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $mainImage = $newsContent->filter('.featured_image img');
                if ($mainImage->count()) {
