...
class TechFusionParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    private const LINK = 'https://techfusion.ru/feed/';
    ...
    public static function run(): array
    {
        ...
        if (count($items)) {
            ...
            foreach ($items as $item) {
                ...
                $image = $originalParser->find('.mkdf-post-image img')->attr('src');
    ...
    {
        $paragraphs = $parser->find('.mkdf-post-text p');
        if (count($paragraphs)) {
