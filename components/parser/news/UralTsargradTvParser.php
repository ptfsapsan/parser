 ...
 * @fullhtml
 */
class UralTsargradTvParser extends MediasferaNewsParser implements ParserInterface
{
    ...
    public const SITE_URL = 'https://ural.tsargrad.tv/';
    public const NEWSLIST_URL = 'https://ural.tsargrad.tv/news';
    ...
    public const ARTICLE_BREAKPOINTS = [
        ...
        'href' => [
            'mailto:ural-grad@mail.ru' => false,
        ]
    ];
            ...
            if (!empty($articleContent)) {
                ...
                foreach ($newsPost->items as $key => $item) {
                    ...
                    if($item->type == NewsPostItem::TYPE_TEXT) {
                        if(strpos($item->text, 'К читателям') === 0) {
                            unset($newsPost->items[$key]);
                            $removeNext = true;
                            continue;
                        }
                        elseif(strpos($item->text, 'Хотите рассказать нам о чем-то важном или высказать свое мнение') !== false) {
                            unset($newsPost->items[$key]);
                            $removeNext = true;
                            continue;
                        }
