...
class OblTvParser implements ParserInterface
{
    /*run*/
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://www.1obl.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $image = $crawler->filter("div.main-cont-block div.newsMediaContainer img");
        if ($image->count() !== 0) {
            $post->image = self::ROOT_SRC . self::normalizeUrl($image->attr("data-src"));
        }

        $body = $crawler->filter("div.main-cont-block div.articleBody");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("p") && $node->filter("span.quote_text")->count() !== 0) {
