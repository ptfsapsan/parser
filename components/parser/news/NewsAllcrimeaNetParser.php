 ...
 * @rss_html
 */
class NewsAllcrimeaNetParser extends MediasferaNewsParser implements ParserInterface
{
    ...
    public const SITE_URL = 'https://news.allcrimea.net/';
    public const NEWSLIST_URL = 'https://allcrimea.net/news/rss.xml';
    ...
    public const ARTICLE_TEXT = '#telonews #telo_right';
    ...
    protected static function parseNode(Crawler $node, ?string $filter = null): void
    {
        ...
        if($node->nodeName() == 'p') {
            $text = trim($node->text());
            if(strpos($text, '«') === 0 && (substr_count($text, '», – ') || substr_count($text, '», - '))) {
                static::$post->itemQuote = $text;
                return;
            }
            elseif(strpos($text, 'В тексте использованы материалы:') === 0) {
                self::$post->stopParsing = true;
                return;
            }
            elseif(strpos($text, 'Ранее Новости Крыма сообщали:') === 0) {
                self::$post->stopParsing = true;
                return;
            }
        }
