 ...
 *
 */
class TkgorodRu extends Aleks007smolBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
     ...
     */
    const MAIN_PAGE_URI = 'http://tkgorod.ru';
     ...
     */
    const FEED_URL = 'http://tkgorod.ru/feed.rss';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function ($node) use (&$curl, &$posts) {
            $newPost = new NewsPost(
                ...
                self::prepareDescription($node->filter('description')->text()) ?: 'description',
            ...
            /**
             * Если встретился определенный элемент, до удаляем всё, что ниже него
             */
            $flag = false;
            $delimeter = '';

            foreach ($newPost->items as $i => $item) {
                if ($item->text == '------------') {
                    $flag = true;
                    $delimeter = $i;
                    break;
            ...
            if ($flag) {
                $count = count($newPost->items);

                for ($i = $delimeter; $i < $count; $i++) {
                    unset($newPost->items[$i]);
                }
            }
