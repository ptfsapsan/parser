...
class BryanskiyRabochiyRfParser extends AbstractBaseParser
{
    ...
    {
        return 'http://xn----8sbbc6addec3arqfj2hrf.xn--p1ai/';
    }
    ...
    protected function parseNewsPage(PreviewNewsDTO $previewNewsDTO): NewsPost
    {
        ...
        $contentCrawler = $newsPageCrawler->filter('#newsWrapper');
        ...
        $descriptionCrawler = $contentCrawler->filterXPath('//div[contains(@class,"description")]');
        if ($this->crawlerHasNodes($descriptionCrawler)) {
            $descriptionText = Text::trim($this->normalizeSpaces($descriptionCrawler->text()));
            if ($descriptionText) {
                $description = $descriptionText;
                $this->removeDomNodes($contentCrawler, '//div[contains(@class,"description")]');
            }
        }

        $contentCrawler = $contentCrawler->filter('.post');
