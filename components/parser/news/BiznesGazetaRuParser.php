...
class BiznesGazetaRuParser implements ParserInterface
{
    public const USER_ID = 2;
    public const FEED_ID = 2;
    private const SITE_URL = 'https://biznes-gazeta.ru';
    ...
    private function getPreviewList(int $minNewsCount = 10, int $maxNewsCount = 100): array
    {
        ...
        $previewNewsCrawler = $previewNewsCrawler->filterXPath('//div[contains(@class,"module-list_news")]//ul/li');
        ...
        $previewNewsCrawler->each(function (Crawler $newsPreview) use (&$previewList, $maxNewsCount) {
            ...
            [$publishedAt, $title] = explode(' // ', $title);
    ...
    private function parseNewsPage(PreviewNewsDTO $previewNewsItem): NewsPost
    {
        ...
        $mainImageCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"shorttext")]//img[1]');

        if ($this->crawlerHasNodes($mainImageCrawler)) {
            $image = $mainImageCrawler->attr('src');
            $this->removeDomNodes($newsPageCrawler, '//div[contains(@class,"shorttext")]//img[1]');
        }
        ...
        $contentCrawler = $newsPageCrawler->filterXPath('//div[contains(@class,"shorttext")]');

        $titleCrawler = $newsPageCrawler->filterXPath('//h1');
        $this->removeDomNodes($titleCrawler, '//span[contains(@class,"date")]');
        $title = Text::trim($titleCrawler->text());

        $description = $newsPageCrawler->filterXPath('//h1/following-sibling::p[contains(@class,"lid")]')->text();
        $this->removeDomNodes($newsPageCrawler, '//h1/following-sibling::p[contains(@class,"lid")]');
